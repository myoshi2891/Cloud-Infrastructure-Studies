...
/**
 * The DevOps Handbook 完全ガイド用のサイドバーナビゲーションコンポーネント。
 * 目次リンクの表示、IntersectionObserver によるスクロールスパイ、キーボードアクセシビリティを提供します。
 */
export const NavBar: FC<NavBarProps> = ({ isOpen = false, onClose }) => {
    const [activeId, setActiveId] = useState<string>(NAV_ITEMS[0]?.id ?? '1-本書の位置づけと全体像');
    ...
    return (
        <aside className={`sidebar${isOpen ? ' open' : ''}`} id="sidebar">
            <div className="sidebar-header">
                <div className="kicker">The DevOps Handbook</div>{' '}
                <h2>初学者向け完全ガイド</h2>
            </div>{' '}
