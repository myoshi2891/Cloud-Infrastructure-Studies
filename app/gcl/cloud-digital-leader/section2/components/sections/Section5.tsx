...
export const Section5: React.FC = () => {
    ...
    return (
        <div id="s5" className={baseStyles.sgap}>
            <div className={baseStyles.secHead}>
                <div className={`${baseStyles.secNum} ${styles.sn5}`}>05</div>
                <div className={baseStyles.secHeadTxt}>
                    <h2 className={baseStyles.secTitle}>Google Cloud のデータストレージサービス</h2>
                </div>
            </div>
            <SectionCard id="cdl-2-5" idNumber="5" title="Google Cloud のデータストレージサービス">
                <h3 className={baseStyles.stitle}>5.1 Cloud Storage（オブジェクトストレージ）</h3>
                <h4 className="stitle mt-4">Cloud Storage とは</h4>
                <p className={baseStyles.tdesc}><strong>Cloud Storage</strong> は、あらゆる種類のファイル（オブジェクト）を
...
主な用途:
  ...
  - データレイクの基盤`}</code></pre>
                ...
                <p className={baseStyles.tdesc}>ストレージクラスは「アクセス頻度」と「コスト」のバランスで選択します。</p>

                <DiagramSVG viewBox="0 0 700 220" ariaLabel="Cloud Storage ストレージクラス比較図: Standard（高アクセス頻度・保存コスト高・取得無料）、Nearline（月1回以下）、Coldline（四半期1回以下）、Archive（年1回以下・最安・取得コスト最高）">
                    <text x="350" y="30" textAnchor="middle" fill="currentColor" fontSize="14">アクセス頻度</text>
                    <path d="M 350 40 L 350 60" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    ...
                    <path d="M 280 135 L 300 135" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="360" y="140" textAnchor="middle" fill="currentColor" fontSize="13">低い</text>
                    <path d="M 390 135 L 410 135" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="470" y="140" textAnchor="middle" fill="currentColor" fontSize="13">さらに低い</text>
                    <path d="M 510 135 L 520 135" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    ...
                    <path d="M 280 160 L 300 160" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="360" y="165" textAnchor="middle" fill="currentColor" fontSize="13">課金</text>
                    <path d="M 390 160 L 410 160" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="470" y="165" textAnchor="middle" fill="currentColor" fontSize="13">課金大きめ</text>
                    <path d="M 510 160 L 520 160" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    ...
                    <path d="M 280 185 L 300 185" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="360" y="190" textAnchor="middle" fill="currentColor" fontSize="13">30日</text>
                    <path d="M 390 185 L 410 185" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                    <text x="470" y="190" textAnchor="middle" fill="currentColor" fontSize="13">90日</text>
                    <path d="M 510 185 L 520 185" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                ...
                />
                <p className={baseStyles.tdesc}>※ 価格はリージョンにより異なります — 表は us-central1 の参考値です。最新情報は<a href="https://cloud.google.com/storage/pricing">公式の料金ページ</a>をご確認ください。</p>

                <h4 className="stitle mt-4">Cloud Storage のライフサイクル管理</h4>
                <DiagramSVG viewBox="0 0 600 200" ariaLabel="Cloud Storage ライフサイクル管理図: 30日後にStandard→Nearline、90日後にNearline→Coldline、365日後にColdline→Archive、7年後に自動削除">
...
コスト最適化:
  ...
  - Recommender の提案を定期的に確認する`}</code></pre>
            ...
            <SectionCard id="gcdl-2-5" idNumber="Deep Dive" title="オブジェクトストレージの最適化：Cloud Storage">
                <p className={baseStyles.tdesc}><strong>2.2 Google Cloud データマネジメントソリューション</strong></p>
                ...
                <p className={baseStyles.tdesc}>最近のトレンドとして、データレイクの柔軟性とデータウェアハウスの管理機能（ACIDトランザクションや高度なSQLサポート）を融合させた「データレイクハウス（Data Lakehouse）」という概念も普及しており、Google CloudではBigQueryの拡張機能（BigLake等）がこの領域をカバーしている。</p>
                
                <h3 className={`${baseStyles.stitle} mt-6`}>オブジェクトストレージの最適化：Cloud Storage</h3>
                <p className={baseStyles.tdesc}>Cloud Storageは、あらゆる量の非構造化データを保存し、必要な頻度で取得できるフルマネージドのオブジェクトストレージサービスである。クラウドのコスト最適化において、Cloud Storageの適切な「ストレージクラス」の選択は極めて重要である。Google Cloudは、データのアクセス頻度と保存期間の要件に応じて、4つの主要なストレージクラスを提供している。</p>
                <p className={baseStyles.tdesc}>第一の「Standard Storage」は、頻繁にアクセスされるホットデータ、アクティブなアプリケーションのバックエンド、ウェブサイトのコンテンツ配信などに最適である。ストレージ単価は他のクラスより高いものの、データの取得コストが無料であり、最低保存期間の要件もない。第二の「Nearline Storage」は、平均して月に1回以下の頻度でアクセスされるデータのバックアップや、長期保存されるマルチメディアコンテンツに適している。最低保存期間は30日である。第三の「Coldline Storage」は、四半期に1回以下の頻度でアクセスされるデータ向けであり、最低保存期間は90日に設定されている。第四の「Archive Storage」は、法令遵守のための監査ログ保存や災害復旧（DR）用途など、1年に1回未満しかアクセスされないデータの長期保管に最適である。ストレージ単価は最も安価であるが、データ取得時の課金が最も高く、最低保存期間は365日に設定されている。</p>
                <p className={baseStyles.tdesc}>Cloud Storageを運用する際のベストプラクティスとして、ライフサイクルルールの積極的な活用が挙げられる。データの価値やアクセスパターンは時間とともに変化する。ある時点では頻繁にアクセスされていたStandardクラスのデータも、半年後には全くアクセスされなくなることがある。バケットごとに適切なライフサイクルポリシーを設定し、一定期間経過後に自動的にNearlineやArchiveへと移行させることで、ユーザーは意識することなく大幅なコスト削減を実現できる。</p>
                <p className={baseStyles.tdesc}>また、Googleアカウントを持たない外部のユーザーに対して、安全かつ一時的にオブジェクトへのアクセス（読み取りやアップロード）を許可したい場合は、「署名付きURL（Signed URLs）」を使用することがベストプラクティスである。これにより、IAMアカウントを個別に作成することなく、有効期限付きのセキュアなアクセス経路を提供できる。</p>
            </SectionCard>
        </div>
    );
};
