    ...
    return (
        <div id="s8" className="sgap">
            ...
            <SectionCard id="cdl-2-8" idNumber="8" title="データパイプラインとデータ統合">
                ...
                <div dangerouslySetInnerHTML={{ __html: `<h3>8.1 Google Cloud Pub/Sub（メッセージング）</h3>
<h4>Pub/Sub とは</h4>
...
Pub/Sub ありの解決策:
  ...
  ✅ 各サービスが独立して動作（疎結合）`}
                ...
                <div dangerouslySetInnerHTML={{ __html: `<h4>Pub/Sub のユースケース</h4>
<table>
<thead>
<tr>
<th>ユースケース</th>
...
<h3>8.2 Cloud Dataflow（データパイプライン）</h3>
<h4>Dataflow とは</h4>
...
典型的な処理フロー:
  ...
  Cloud SQL  →   フォーマット変換        →   Pub/Sub`}
                ...
                <div dangerouslySetInnerHTML={{ __html: `<h4>Dataflow のユースケース</h4>
<table>
<thead>
<tr>
<th>ユースケース</th>
...
<h3>8.3 Cloud Dataproc（マネージド Hadoop/Spark）</h3>
<h4>Dataproc とは</h4>
...
` }} className="prose max-w-none text-sm mb-4" />
            ...
            <SectionCard id="gcdl-2-8" idNumber="Deep Dive" title="ストリーミング分析アーキテクチャ: Pub/Sub と Dataflow">
                ...
                <div dangerouslySetInnerHTML={{ __html: `<h3><strong>ストリーミング分析アーキテクチャ: Pub/Sub と Dataflow</strong></h3>
<p>現代のビジネスにおいて、データが生成されたその瞬間にインサイトを抽出し、行動に結びつける「リアルタイムストリーミング分析」の重要性はますます高まっている 。Google Cloudにおいて、このストリーミング分析のパイプラインは、「Cloud Pub/Sub」と「Cloud Dataflow」の強力な連携によって実現される 。</p>
<p>「Cloud Pub/Sub」は、独立したアプリケーション間でイベントデータを非同期に送受信する、グローバルに分散されたフルマネージドのメッセージング指向ミドルウェアである 。データの送信者（パブリッシャー）と受信者（サブスクライバー）を論理的に切り離し（Decoupling）、突発的なトラフィックのスパイク（急増）が発生した場合でも、オートスケーリングによってデータを確実に取り込み、バッファリングする「ショックアブソーバー（緩衝材）」の役割を果たす 。</p>
...
<p>このPub/SubとDataflowを統合して堅牢なストリーミングパイプラインを構築する際には、システムアーキテクチャにおける重要なベストプラクティスを遵守する必要がある 。</p>
<p>第一に、「Exactly-once（1回限り）処理」の重複排除メカニズムの競合を避けることである。Dataflowは、パイプライン内で独自のメカニズムを用いてメッセージの重複を排除し、強固なExactly-onceセマンティクスを保証している。そのため、データソースであるPub/Sub側の機能として提供されている「Exactly-once配信」を有効にしてDataflowと接続することは推奨されない 。両方のシステムで厳密な配信保証を行おうとすると、並列処理できるメッセージ数が著しく制限され、パイプライン全体のパフォーマンス低下やスループットの悪化を招くためである 。</p>
