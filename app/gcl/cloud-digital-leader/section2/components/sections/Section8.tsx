...
export const Section8: React.FC = () => {
    return (
        <div id="s8" className={styles.sgap}>
            <div className={styles.secHead}>
                <div className={`${styles.secNum} ${styles.sn8}`}>08</div>
                <div className={styles.secHeadTxt}>
                    <h2 className={styles.secTitle}>データパイプラインとデータ統合</h2>
                </div>
            </div>
            <SectionCard id="cdl-2-8" idNumber="8" title="データパイプラインとデータ統合">
                <h3 className={styles.stitle}>8.1 Google Cloud Pub/Sub（メッセージング）</h3>
                <h4 className="stitle mt-4">Pub/Sub とは</h4>
                <p className={styles.tdesc}><strong>Pub/Sub</strong>（パブリッシュ・サブスクライブ）は、
                ...
                <h4 className={`${styles.stitle} mt-6`}>Pub/Sub を使う理由</h4>
                ...
                <h4 className={`${styles.stitle} mt-6`}>Pub/Sub のユースケース</h4>
                <TableComponent
                    headers={['ユースケース', '説明']}
                ...
                <h3 className={`${styles.stitle} mt-6`}>8.2 Cloud Dataflow（データパイプライン）</h3>
                <h4 className="stitle mt-4">Dataflow とは</h4>
                <p className={styles.tdesc}><strong>Dataflow</strong> は、バッチ処理とストリーミング処理の両方に対応した
...
典型的な処理フロー:
  ...
  Cloud SQL  →   フォーマット変換        →   Pub/Sub`}</code></pre>
                ...
                <h3 className={`${styles.stitle} mt-6`}>8.3 Cloud Dataproc（マネージド Hadoop/Spark）</h3>
                <h4 className="stitle mt-4">Dataproc とは</h4>
                <p className={styles.tdesc}><strong>Dataproc</strong> は、Apache Hadoop・Apache Spark クラスタを
...
選択基準:
  ...
  新規パイプライン設計（特にストリーミング） → Dataflow`}</code></pre>
                ...
                <h3 className={`${styles.stitle} mt-6`}>8.4 Database Migration Service（データ移行）</h3>
                ...
                <h3 className={`${styles.stitle} mt-6`}>8.5 Datastream（変更データキャプチャ）</h3>
                <DiagramSVG viewBox="0 0 600 160">
                    <rect x="10" y="10" width="580" height="140" rx="8" strokeWidth="2" stroke="var(--color-primary, currentColor)" fill="var(--color-background, transparent)" />
            ...
            <SectionCard id="gcdl-2-8" idNumber="Deep Dive" title="ストリーミング分析アーキテクチャ: Pub/Sub と Dataflow">
                <h3 className={`${styles.stitle} mt-6`}>ストリーミング分析アーキテクチャ: Pub/Sub と Dataflow</h3>
                <p className={styles.tdesc}>現代のビジネスにおいて、データが生成されたその瞬間にインサイトを抽出し、行動に結びつける「リアルタイムストリーミング分析」の重要性はますます高まっている。Google Cloudにおいて、このストリーミング分析のパイプラインは、「Cloud Pub/Sub」と「Cloud Dataflow」の強力な連携によって実現される。</p>
                <p className={styles.tdesc}>「Cloud Pub/Sub」は、独立したアプリケーション間でイベントデータを非同期に送受信する、グローバルに分散されたフルマネージドのメッセージング指向ミドルウェアである。データの送信者（パブリッシャー）と受信者（サブスクライバー）を論理的に切り離し（Decoupling）、突発的なトラフィックのスパイク（急増）が発生した場合でも、オートスケーリングによってデータを確実に取り込み、バッファリングする「ショックアブソーバー（緩衝材）」の役割を果たす。</p>
                <p className={styles.tdesc}>一方、「Cloud Dataflow」は、Apache Beam SDKを基盤とするサーバーレスのデータ処理サービスである。ストリーミングデータとバッチデータの両方を全く同じプログラミングモデルで処理でき、Pub/Subからリアルタイムに送られてくるメッセージを抽出、変換、集計（ETL）し、最終的な分析先であるBigQueryなどにロードする役割を担う。Dataflowは、データの到着遅延を処理するウォーターマーク機能や、ウィンドウ処理機能に優れている。</p>
                <p className={styles.tdesc}>このPub/SubとDataflowを統合して堅牢なストリーミングパイプラインを構築する際には、システムアーキテクチャにおける重要なベストプラクティスを遵守する必要がある。</p>
                <p className={styles.tdesc}>第一に、「Exactly-once（1回限り）処理」の重複排除メカニズムの競合を避けることである。Dataflowは、パイプライン内で独自のメカニズムを用いてメッセージの重複を排除し、強固なExactly-onceセマンティクスを保証している。そのため、データソースであるPub/Sub側の機能として提供されている「Exactly-once配信」を有効にしてDataflowと接続することは推奨されない。両方のシステムで厳密な配信保証を行おうとすると、並列処理できるメッセージ数が著しく制限され、パイプライン全体のパフォーマンス低下やスループットの悪化を招くためである。</p>
                <p className={styles.tdesc}>第二に、システムコストとレイテンシの最適化である。もしビジネス要件として、後段のシステム（例えばBigQuery側）でデータの重複を許容できる、あるいは独自に重複排除を行える設計であるならば、DataflowのストリーミングモードをデフォルトのExactly-onceから「At-least-once（少なくとも1回）」モードに変更することが推奨される。これにより、内部的な重複排除処理のオーバーヘッドが削減され、処理レイテンシの向上とコンピューティングコストの大幅な削減を実現できる。</p>
                <p className={styles.tdesc}>第三に、Pub/Subのサブスクリプションとデッドレタートピックの適切な構成である。一つのPub/Subサブスクリプションを複数のDataflowパイプラインで共有して読み取ることは避けるべきである。これを実行すると、データが非決定的に各パイプラインに分割されてしまい、重複メッセージの発生やウォーターマーク処理の遅延、オートスケーリングの非効率化を引き起こすため、必ずパイプラインごとに独立したサブスクリプションを作成する必要がある。また、処理に失敗したメッセージを退避させるためのPub/Subの「デッドレタートピック」機能は、Dataflowのソースとして設定するべきではない。Dataflowはワーカーのシャットダウンなどの内部的な理由により、パイプラインのコード自体にエラーがなくてもPub/Subに対して否定応答（NACK）を返すことがあるため、正常なメッセージまで誤ってデッドレタートピックに送られてしまうリスクがあるからだ。</p>
                <p className={styles.tdesc}>さらに、稼働中のDataflowパイプラインにおいて、Pub/Subの過去の特定時点に巻き戻してデータを再読み込みする機能（Pub/Sub Seek）を使用することは推奨されない。ウォーターマークのロジックが破綻し、大量のメッセージの重複や欠落を引き起こす原因となる。データを再処理する必要がある場合は、サブスクリプションのスナップショットを作成し、新しいサブスクリプションをそこから派生させた上で、既存のパイプラインをドレイン（安全な停止）し、新しいサブスクリプションをソースとして新規パイプラインを立ち上げるというワークフローがベストプラクティスである。</p>
            </SectionCard>
        </div>
    );
};
