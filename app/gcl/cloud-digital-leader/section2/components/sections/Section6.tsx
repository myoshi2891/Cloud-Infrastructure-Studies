...
export const Section6: React.FC = () => {
    ...
    return (
        <div id="s6" className={baseStyles.sgap}>
            <div className={baseStyles.secHead}>
                <div className={`${baseStyles.secNum} ${styles.sn6}`}>06</div>
                <div className={baseStyles.secHeadTxt}>
                    <h2 className={baseStyles.secTitle}>Google Cloud のデータベースサービス</h2>
                </div>
            </div>
            <SectionCard id="cdl-2-6" idNumber="6" title="Google Cloud のデータベースサービス">
                <h3 className={baseStyles.stitle}>6.1 データベース選択の全体像</h3>
                <p className={baseStyles.tdesc}>Google Cloud には多種多様なデータベースサービスがあります。
   ...
   Spanner   Cloud SQL  Firestore Bigtable Memorystore
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.2 Cloud SQL（マネージドリレーショナルDB）</h3>
...
制限・注意点:
  ...
  ❌ 数十 TB を超える大規模分析には BigQuery が適切`}</code></pre>
                ...
                <DiagramSVG viewBox="0 0 600 240" ariaLabel="Cloud SQL の HA 構成図: シングルゾーン構成（SLA 99.95%）とマルチゾーン構成（SLA 99.99%、自動フェイルオーバー60秒）の比較">
                    ...
                    <path d="M 210 205 L 250 205" stroke="currentColor" markerEnd={`url(#${markerId})`} />
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.3 Cloud Spanner（グローバル分散 RDBMS）</h3>
                <DiagramSVG viewBox="0 0 700 240" ariaLabel="Cloud Spanner の機能図: グローバル分散・強一貫性RDB、SLA 99.999%、グローバルEC・金融システムのユースケース">
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.4 Firestore（NoSQL ドキュメント DB）</h3>
...
特徴:
  ...
  ❌ 大規模な JOIN クエリや集計分析は得意でない`}</code></pre>
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.5 Bigtable（NoSQL ワイドカラム DB）</h3>
...
特徴:
  ...
  ❌ 小規模データには向かない（最低 1 TB 程度から真価を発揮）`}</code></pre>
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.6 BigQuery（データウェアハウス）</h3>
...
コスト最適化のポイント:
  ...
  - クラスタリングを活用（よく使うカラムでソート）`}</code></pre>
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.7 Memorystore（インメモリ DB）</h3>
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>6.8 AlloyDB（PostgreSQL 互換高性能 DB）</h3>
  ...
  - 完全 PostgreSQL 互換（既存のコード・ツールがそのまま動く）`}</code></pre>
            ...
            <SectionCard id="gcdl-2-6" idNumber="Deep Dive" title="リレーショナルデータベースとNoSQLの戦略的活用">
                <h3 className={`${baseStyles.stitle} mt-6`}>リレーショナルデータベースの選定とアーキテクチャ</h3>
                <p className={baseStyles.tdesc}>リレーショナルデータベース（RDBMS）は、データがテーブル、行、列の形式で構造化され、SQLを使用して操作されるシステムである。金融取引や在庫管理など、トランザクションの原子性、一貫性、独立性、永続性（ACID特性）が厳密に求められるユースケースにおいて不可欠である。Google Cloudでは主に二つの強力なRDBMSの選択肢が存在する。</p>
                <p className={baseStyles.tdesc}>一つ目は「Cloud SQL」である。これは、MySQL、PostgreSQL、SQL Serverといったオープンソースおよび商用のデータベースエンジンをフルマネージドで提供するサービスである。データベースのプロビジョニング、バックアップ、パッチ適用、フェイルオーバーといった日常的な運用タスクが自動化されている。アプリケーションのバックエンドとして、データ容量が数十TB未満であり、単一リージョンでの高可用性で十分な一般的なトランザクション処理（OLTP）システムにおいて、最も標準的で推奨される選択肢である。</p>
                <p className={baseStyles.tdesc}>二つ目は「Cloud Spanner」である。これは、リレーショナルデータベースの利点（完全なSQLサポート、厳密なグローバルACID整合性）と、非リレーショナルデータベースの利点（無限の水平スケーリング）を世界で初めて統合した革新的なグローバル分散型データベースである。Cloud SQLが垂直スケーリング（インスタンスのスケールアップ）に限界を持つのに対し、Cloud Spannerはノードを追加するだけでダウンタイムなしに書き込みと読み取りのパフォーマンスを線形にスケールアウトできる。データ量が10TBを大きく超えるシステムや、グローバルな金融決済システム、最大99.999%の可用性が求められるミッションクリティカルな環境において最適なソリューションである。</p>
                <p className={baseStyles.tdesc}>Cloud Spannerのパフォーマンスを最大限に引き出すためのベストプラクティスは、従来の単一ノードRDBMSの常識とは大きく異なる点に注意が必要である。Spannerは背後でデータをキーに基づいて複数のサーバー（スプリット）に分散・パーティショニングする。そのため、タイムスタンプや連続するシーケンス番号（AUTO_INCREMENTなど）を主キー（Primary Key）として使用すると、すべての新しいデータが単一のサーバーに集中して書き込まれる「ホットスポット（Hotspotting）」という現象が発生し、パフォーマンスが著しく低下する。これを回避するためには、UUIDバージョン4などのランダムな値を主キーとして採用するか、連続するキーのビットを反転（Bit-reverse）させて保存空間全体に書き込み負荷を均等に分散させるスキーマ設計が強く推奨される。</p>

                <h3 className={`${baseStyles.stitle} mt-6`}>非リレーショナルデータベース (NoSQL) の戦略的活用</h3>
                <p className={baseStyles.tdesc}>非リレーショナルデータベース（NoSQL）は、厳密なテーブル構造を持たず、柔軟なスキーマを許容することで、極めて大規模なデータの読み書きを驚異的な低レイテンシで処理することに特化している。</p>
                <p className={baseStyles.tdesc}>大規模な分析および運用ワークロード向けに設計されているのが「Cloud Bigtable」である。Bigtableは、ペタバイト規模のデータをミリ秒未満のレイテンシで処理できるフルマネージドのワイドカラム（Wide-column）型NoSQLデータベースである。IoTデバイスから絶え間なく送信されるセンサーデータ、アドテクにおけるリアルタイムのユーザー行動ログ、金融市場の時系列ティッカーデータなど、極端に高いスループットとスケーラビリティが要求されるユースケースに最適である。</p>
                <p className={baseStyles.tdesc}>Bigtableのパフォーマンスは、スキーマ設計、特に「行キー（Row Key）」の設計に完全に依存している。Bigtableのベストプラクティスとして、関連するデータは可能な限り単一の行に格納することが推奨されるが、1行あたりのデータサイズが100MBを超えないように分割する必要がある。また、行キーはメモリとストレージ効率のために4KB以下に短く保つべきである。時系列データを扱う場合は、ハッシュ関数を用いて生成したプレフィックスを行キーの先頭に付加する「キーソルト（Key Salting）」という手法を用いることで、分散ノード全体に書き込み負荷を均等に分散させることがベストプラクティスとして確立されている。</p>
                ...
                <h3 className={`${baseStyles.stitle} mt-6`}>データベースの移行とモダナイゼーション (Database Migration)</h3>
                <p className={baseStyles.tdesc}>オンプレミスのレガシーデータベースや他のクラウドプロバイダーの環境からGoogle Cloudへの移行は、インフラストラクチャをモダナイズする上で不可避のステップである。この移行プロセスにおける運用上の負担を排除し、安全かつシームレスな移行を実現するサービスが「Database Migration Service (DMS)」である。</p>
            </SectionCard>
        </div>
    );
};
