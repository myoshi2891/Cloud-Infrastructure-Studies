...
<h4 class="stitle">Cloud SQL の高可用性（HA）構成</h4>
` }} />
...
本番環境では必ず HA 構成を使用すること！`}
                ...
                <div dangerouslySetInnerHTML={{ __html: `<h3 class="stitle">6.3 Cloud Spanner（グローバル分散 RDBMS）</h3>
` }} />
...
<p class="tdesc">さらに、SaaSアプリケーション（Salesforceなど）、クラウドストレージ（Amazon S3など）、あるいはSnowflakeやTeradataといった他のデータウェアハウスから、Google CloudのBigQueryへ定期的にデータを転送・統合する場合には、「BigQuery Data Transfer Service (DTS)」を利用することがベストプラクティスである 。DTSは、コーディングを必要とせずにデータ転送のスケジューリングを自動化し、フルロードだけでなく増分移行にも対応するため、データ分析基盤の構築を迅速化する 。</p>
` }} />
