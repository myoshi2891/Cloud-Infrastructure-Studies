    ...
    return (
        <div id="s7" className="sgap">
            ...
            <SectionCard id="cdl-2-7" idNumber="7" title="データ分析・BI サービス">
                <h3 className="stitle">7.1 Looker（エンタープライズ BI プラットフォーム）</h3>
                <h4 className="stitle mt-4">Looker とは</h4>
                <p className="tdesc"><strong>Looker</strong> は、Google Cloud のエンタープライズ向け
                ...
                <h4 className="stitle mt-6">LookML とは</h4>
                ...
                <h4 className="stitle mt-6">Looker の主な機能</h4>
                <TableComponent
                ...
                <h3 className="stitle mt-6">7.2 Looker Studio（無料セルフサービス BI）</h3>
                <h4 className="stitle mt-4">Looker Studio とは</h4>
                <p className="tdesc"><strong>Looker Studio</strong>（旧 Google Data Studio）は、
...
接続できるデータソース（主要なもの）:
  ...
  - + サードパーティコネクタ多数`}</code></pre>
                ...
                <h3 className="stitle mt-6">7.3 Looker vs Looker Studio の比較</h3>
                <p className="tdesc">試験で最もよく問われる比較ポイント!</p>
...
Looker Studio を選ぶ場合:
  ...
  - Google Analytics・広告データの可視化`}</code></pre>
            ...
            <SectionCard id="gcdl-2-7" idNumber="Deep Dive" title="データの有用性とアクセシビリティの向上 / BigQueryとLooker">
                <p className="tdesc"><strong>2.3 データの有用性とアクセシビリティの向上 (Making Data Useful and Accessible)</strong></p>
                <p className="tdesc">データは適切に保存されるだけでは価値を生まない。リアルタイムの分析を通じて、組織内のあらゆるユーザーがデータにアクセスし、インサイトを引き出せる状態（データの民主化）を作り出すことが、真のデジタルトランスフォーメーションの要である。</p>

                <h3 className="stitle mt-6">BigQuery: サーバーレスデータウェアハウスの真髄</h3>
                <p className="tdesc">Google Cloudのデータ戦略の中核に位置するのが「BigQuery」である。BigQueryは、ペタバイト規模のデータを扱うサーバーレスのフルマネージド・データウェアハウスであり、機械学習（ML）機能を内蔵した自律型のデータ・トゥ・AIプラットフォームとして機能する。従来のデータウェアハウスのようにインフラストラクチャのサイジングやクラスタの管理、インデックスの作成といった運用タスクは一切不要である。</p>
                <p className="tdesc">BigQueryのアーキテクチャの最大の強みは、「コンピューティング（クエリの処理能力）」と「ストレージ（データの保存場所）」が物理的かつ論理的に完全に分離されている（Decoupled Architecture）点にある。この分離により、ユーザーは保存しているデータの総量に関係なく、必要な時に必要なだけのコンピューティングリソースを動的にスケールさせてクエリを実行できる。これにより、パフォーマンスのボトルネックが解消されると同時に、使用した分だけ課金されるという極めて高いコスト効率の「OpEx（運用支出）モデル」が実現されている。</p>

                <h3 className="stitle mt-6">BigQuery Omni によるマルチクラウド分析の実現</h3>
                <p className="tdesc">現代のエンタープライズ企業の90%以上が、単一のクラウドプロバイダーに依存しないマルチクラウド戦略を採用している。しかし、その結果としてデータがAmazon Web Services (AWS) やMicrosoft Azureといった異なるクラウド環境にサイロ化されるという新たな課題に直面している。通常、これらの外部クラウドからデータを抽出し、分析のために別の中央プラットフォームに移動させるには、膨大な時間と高額な「ネットワークエグレス（下り）料金」が発生する。</p>
                <p className="tdesc">このマルチクラウドの課題を根本から解決する革新的なソリューションが「BigQuery Omni」である。BigQuery Omniは、Google Cloudのハイブリッド・マルチクラウドプラットフォームである「Anthos」テクノロジーを基盤としており、BigQueryの強力なクエリエンジンそのものをAWSやAzureの環境内で直接稼働させるアーキテクチャを採用している。</p>
                <p className="tdesc">BigQuery Omniの最大のビジネス価値は、AWSのAmazon S3やAzure Blob Storageに保存されているデータを、Google Cloudに一切移動またはコピーすることなく、使い慣れたBigQueryのコンソール画面から標準のGoogleSQLを用いて直接クエリできることである。これにより、高額なデータ転送コスト（エグレス料金）を完全に回避しつつ、組織全体に分散したデータセットに対するクロス・クラウド分析（Cross-cloud joins）が可能となり、分散したデータガバナンスのオーバーヘッドを削減しながら、統一された分析体験を実現できる。</p>

                <h3 className="stitle mt-6">Looker によるビジネスインテリジェンスとデータの民主化</h3>
                <p className="tdesc">データが統合・処理された後、それを組織内の誰もがアクセス可能で、視覚的に理解できる「インサイト」へと昇華させるのが、エンタープライズ向けビジネスインテリジェンス（BI）プラットフォームである「Looker」の役割である。</p>
                <p className="tdesc">Lookerは従来のBIツールとは根本的に異なるアーキテクチャを採用している。従来のツールは、分析のたびにデータベースから手元のBIサーバーにデータを抽出（Extract）してメモリ上に保持する仕組みをとっていたが、これはデータのサイロ化やセキュリティリスク、そしてデータ鮮度の低下（古いデータを見てしまう問題）を引き起こす。Lookerは、データを抽出しない「インデータベース（In-database）アーキテクチャ」を採用している。ユーザーがダッシュボード上でフィルタリングやドリルダウンの操作を行うと、Lookerはその背後で最適化された高パフォーマンスなSQL（GoogleSQLなど）を自動生成し、BigQueryなどのデータウェアハウスに直接クエリを投げる。結果として、ユーザーは常に「唯一の真実の情報源（Single Source of Truth）」となる最新のデータに対し、BigQueryの無限のコンピューティングパワーを利用して分析を行うことができる。</p>
                <p className="tdesc">Lookerの中核にあるイノベーションが「LookML」と呼ばれるセマンティックモデリング言語である。データエンジニアやアナリストは、LookMLを使用して売上の計算ロジックや「アクティブユーザー」といったビジネス指標の定義を中央でコードとして一元管理する。これにより、経営層、マーケティング、営業といった異なる部門のユーザーがセルフサービスでデータを探索しても、計算ロジックのブレが生じることなく、常に一貫した正しい結果を得ることが可能となる（データの民主化）。</p>
                <p className="tdesc">大規模なデータワークロードにおけるLookerのパフォーマンス最適化とガバナンスには、いくつかの重要なベストプラクティスが存在する。第一に、数千万行を超えるような巨大なデータセットに対するリアルタイム計算の回避である。行レベルの詳細データを毎回クエリすると、BigQueryの課金とレスポンス遅延が増大する。この問題を解決するために、月次や週次などの単位で事前に集計した「集計テーブル（Pre-Aggregated Data）」を作成し、Lookerの「Aggregate Awareness（集計の認識）」機能を活用することが最も効果的な最適化戦略である。これにより、Lookerはユーザーのクエリが事前集計テーブルで回答可能かどうかを自動的に判断し、適切な場合は軽量なテーブルにクエリをルーティングするため、パフォーマンスが10倍から100倍向上する。</p>
                <p className="tdesc">第二に、データモデルにおける無駄な処理の削減である。分析に不要なテーブルの結合（Join）は極力制限し、ダッシュボード上のExplore（データ探索領域）を焦点を絞ったシンプルな状態に保つことが重要である。ダッシュボードに配置する要素（タイルやグラフ）の数も重要であり、1つのダッシュボードに25個以上の複雑なクエリタイルを配置すると、ブラウザのメモリリソースを著しく消費し、レンダリング速度が低下するため避けるべきである。</p>
                <p className="tdesc">第三に、データガバナンスとアクセス制御の最適化である。Lookerでは行レベルのセキュリティ（Row-Level Security）などを柔軟に設定できるが、ユーザーレベルでの複雑なアクセス制御ロジックを多用すると、クエリの実行計画が複雑化し、システム全体のパフォーマンスに悪影響を及ぼす。したがって、ロールベースのアクセス制御（RBAC）を用いて、ユーザーの役割や責任範囲に必要なExploreやフィールドへのアクセス権のみをシンプルに付与し、不必要な大規模データへの偶発的なクエリを制限することが、セキュリティとパフォーマンスの両面におけるベストプラクティスである。</p>
            </SectionCard>
        </div>
    );
};