...
export const NAV_ITEMS: NavItem[] = [
  { id: 'この章の位置づけ', label: 'この章の位置づけ' },
  {
    ...
    subItems: [
      ...
      { id: '214-google-apisマネージドサービスへのプライベートアクセス構成', label: '2.1.4 Google APIs・マネージドサービスへのプライベートアクセス構成' },
      { id: '215-vpcサブネット範囲の拡張', label: '2.1.5 VPCサブネット範囲の拡張' },
      { id: '216-vpc-service-controls-パリメータの構成', label: '2.1.6 VPC Service Controls パリメータの構成' },
    ],
  },
  {
    id: '22-vpcルーティングの構成',
    ...
    subItems: [
      { id: '221-静的ルーティングと動的ルーティング', label: '2.2.1 静的ルーティングと動的ルーティング' },
      { id: '222-グローバル--リージョナル動的ルーティングモード', label: '2.2.2 グローバル / リージョナル動的ルーティングモード' },
      { id: '223-ネットワークタグ優先度によるルーティング', label: '2.2.3 ネットワークタグ・優先度によるルーティング' },
      { id: '224-グローバル動的ルーティングでのルート優先度ポリシーベースルーティング-vs-bgp-route-policies', label: '2.2.4 グローバル動的ルーティングでのルート優先度:ポリシーベースルーティング vs BGP route policies' },
      { id: '225-内部パススルーnetwork-load-balancerをネクストホップとして利用', label: '2.2.5 内部パススルーNetwork Load Balancerをネクストホップとして利用' },
      { id: '226-vpc-peering--ncc越しのカスタムルートインポートエクスポート', label: '2.2.6 VPC Peering / NCC越しのカスタムルートインポート/エクスポート' },
      { id: '227-ポリシーベースルーティングの構成', label: '2.2.7 ポリシーベースルーティングの構成' },
    ],
  },
  {
    id: '23-network-connectivity-centerの構成',
    label: '2.3 Network Connectivity Centerの構成',
    subItems: [
      { id: '231-スポークタイプとトポロジ管理要点', label: '2.3.1 スポークタイプとトポロジ管理(要点)' },
      { id: '232-nccの監視トラブルシューティング深掘り', label: '2.3.2 NCCの監視・トラブルシューティング(深掘り)' },
    ],
  },
  {
    id: '24-gkeクラスタの構成と維持',
    ...
    subItems: [
      { id: '241-vpc-nativeクラスタalias-ip', label: '2.4.1 VPC-nativeクラスタ(alias IP)' },
      { id: '242-shared-vpcでのクラスタ構成', label: '2.4.2 Shared VPCでのクラスタ構成' },
      { id: '243-プライベートクラスタとプライベートコントロールプレーンエンドポイント', label: '2.4.3 プライベートクラスタとプライベートコントロールプレーンエンドポイント' },
      { id: '244-gke-dataplane-v2', label: '2.4.4 GKE Dataplane V2' },
      { id: '245-snat送信元natとip-masqueradeポリシー', label: '2.4.5 SNAT(送信元NAT)とIP Masqueradeポリシー' },
      { id: '246-podレンジserviceレンジの構成と追加podレンジ', label: '2.4.6 Podレンジ・Serviceレンジの構成と追加Podレンジ' },
      { id: '247-gkeのdns構成', label: '2.4.7 GKEのDNS構成' },
    ],
  },
