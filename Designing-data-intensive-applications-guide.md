...
| リーダーレス | どのノードにも書き込み可能、読み書きクォーラム（w + r > n）で読み取りの鮮度を調整 | 単一障害点がなく、一部ノードが落ちても読み書きを継続しやすい（実際の可用性・耐障害性はレプリケーション係数、読み取り／書き込みクォーラムの設定、修復と競合解決の実装に依存する） | 読み取り時の修復（Read Repair）が必要 | Amazon Dynamo系（Cassandra、Riak） |
