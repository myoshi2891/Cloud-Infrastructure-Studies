...
4. [Part I：データシステムの基礎](#4-part-iデータシステムの基礎)
5. [Part II：データの保存と検索](#5-part-iiデータの保存と検索)
6. [Part III：分散データ](#6-part-iii分散データ)
7. [Part IV：派生データ](#7-part-iv派生データ)
8. [Part V：倫理と社会（第2版新設）](#8-part-v倫理と社会第2版新設)
...
初版（2017年）刊行から約10年が経過した今も、システム設計面接の定番教材、バックエンド/インフラエンジニアの必読書として挙げられ続けています。著名な開発者からも高く評価されており、例えば Apache Kafka の創作者であり Confluent 共同創業者の Jay Kreps は、本書が分散システムの理論と実務の間の大きなギャップを橋渡ししてくれる点を高く評価しています。また Microsoft の CTO である Kevin Scott も、データインフラの設計判断を行うすべてのソフトウェアエンジニアに読んでほしい一冊だとコメントしています（出典は本ガイド末尾を参照）。

さらに、Google でデータベース関連の仕事をしていた経験を持ち、後に Amazon Web Services のプリンシパルエンジニアとなった Jaana Dogan も、分散システムの背景知識が必要な同僚に本書を勧め続けてきたと述べています。
...
```mermaid
flowchart TB
    Start(["DDIA 第2版<br/>全14章 + 序文"])
    Start --> P1
    subgraph P1["Part I: データシステムの基礎"]
        C1["第1章<br/>データシステムのトレードオフ"]
        C2["第2章<br/>非機能要件の定義"]
    end
    subgraph P2["Part II: データの保存と検索"]
        C3["第3章<br/>データモデルとクエリ言語"]
        C4["第4章<br/>ストレージとインデックス"]
        C5["第5章<br/>エンコーディングと進化"]
    end
    subgraph P3["Part III: 分散データ"]
        C6["第6章<br/>レプリケーション"]
        C7["第7章<br/>シャーディング"]
        C8["第8章<br/>トランザクション"]
    ...
    subgraph P4["Part IV: 派生データ"]
        C11["第11章<br/>バッチ処理"]
        C12["第12章<br/>ストリーム処理"]
        C13["第13章<br/>ストリーミングシステムの哲学"]
    end
    subgraph P5["Part V: 倫理と社会"]
        C14["第14章<br/>正しいことをする"]
    end
    P1 --> P2 --> P3 --> P4 --> P5

    classDef partStyle fill:#1b2a4a,stroke:#7c9eff,color:#e8edff
    class P1,P2,P3,P4,P5 partStyle
```

DDIAは大きく5つのパートに分かれており、**基礎概念 → 単一ノードでのデータ保存 → 複数ノードに分散したデータ → 複数のデータストアをまたぐ派生データ → 技術と社会の関係**という順序で理解が積み上がっていく構成になっています。この順序どおりに学習を進めることが、初学者にとって最も挫折しにくいステップです。
...
| Part | 第1版の該当章 | 第2版の該当章 | 主な変更点 |
|---|---|---|---|
| 基礎 | 1章（信頼性・拡張性・保守性） | 1章＋2章に分割・拡張 | クラウド vs 自己ホスティングの章を新設 |
...
| ストリーム処理 | 11章 | 12章 | 変更なし（引き続き中核） |
| 未来の展望 | 12章「データシステムの未来」 | 13章「ストリーミングシステムの哲学」に改題 | Unbundling Databasesの議論を継承・発展 |
| 倫理 | 12章の一節「正しいことをする」 | 14章「正しいことをする」（独立章化） | 予測分析・バイアス・プライバシー／トラッキングは第1版12章にも存在。AI時代を踏まえて再編・大幅加筆 |
...
| 人的エラー | 設定ミス、誤ったデプロイ | ロールバック機構、段階的リリース、Chaos Engineering |
...
| グラフ | 高度に相互接続したデータ（SNS、レコメンド） | Cypher、SPARQL、Datalog、GraphQL | 単純な表形式データには過剰 |

第2版で新たに加わったのは、**GraphQL**（クライアント主導でグラフ状のデータを取得するクエリ言語）と、**DataFrame・行列形式**（機械学習・データ分析パイプラインで多用される表現）です。一方、**Datalog**（再帰的なクエリを表現できる宣言的言語）や**Property Graph**、**Event Sourcing / CQRS**（書き込みと読み取りのモデルを分離する設計パターン）は第1版でも扱われていたテーマであり、第2版では記述の再編・拡張が行われています。
...
第2版では、分析（OLAP）用途に特化した**列指向ストレージ（Column-Oriented Storage）**、**クエリのコンパイルとベクトル化実行**、**マテリアライズドビューとデータキューブ**、そして生成AI時代を反映した**ベクトル埋め込み（Vector Embeddings）**と全文検索の節が新たに追加されています。ベクトル埋め込みはRAG（検索拡張生成）アプリケーションの基盤技術として、近年のデータベース設計で無視できないテーマになっています。
...
| リーダーレス | どのノードにも書き込み可能、クォーラムで整合性を担保 | 単一障害点がない、高可用性 | 読み取り時の修復（Read Repair）が必要 | Amazon Dynamo系（Cassandra、Riak） |
...
> - 業務整合性そのものは、トランザクション分離レベル（少なくともスナップショット分離、競合が絡むなら直列化可能性）、在庫キーやオーダーIDへの一意性制約、Write Skew を防ぐ明示的なロックや制約、そして操作の冪等性キーによる重複要求の排除で保証する
...
flowchart TB
    ...
    L2 --> L3["Snapshot Isolation<br/>/ Repeatable Read"]
...
| Snapshot Isolation | Non-Repeatable Read、大半のLost Update | Write Skew、一部のPhantom | PostgreSQL、MySQL(InnoDB)のRepeatable Read相当 |
| Serializable | ほぼ全ての異常（トランザクションを直列実行したのと同じ結果を保証） | （実装コストとスループットのトレードオフ） | Serializable Snapshot Isolation (SSI) など |
...
**合意アルゴリズム（Consensus）**は、一部のノードが故障しても、生き残った過半数のノードが「ある1つの値」について一致できることを保証する**耐障害性のあるプリミティブ**で、リーダー選出・分散ロック・全順序ブロードキャストなどの基盤になります。代表的な実装にPaxos、Raft、Zabがあり、実務ではこれらを内包したZooKeeperやetcdのような「**コーディネーションサービス**」を直接利用するケースが一般的です。

一方、複数のノードにまたがる書き込みを「全員コミットするか、全員アボートするか」に揃える**2相コミット（2PC）**は、これとは別の抽象化です。2PCは**アトミックコミットプロトコル**であり、コーディネーターが準備フェーズ後に停止すると参加者はロックを握ったまま待ち続ける（**ブロックし得る**）という弱点を持ちます。Consensusはこの点で異なり、過半数さえ生きていれば進行できます。両者は排他的ではなく、コーディネーターの決定ログを合意アルゴリズムで複製して単一障害点を減らす（いわゆる耐障害性のある2PC）という形で**併用される場合があります**。
...
正データ（System of Record）から、検索インデックス・キャッシュ・分析基盤のような「派生データ（Derived Data）」を生成・維持する仕組みを扱うパートです。
...
第2版で特筆すべき変更は、分散ファイルシステム（HDFSなど）と並んで**オブジェクトストア（S3、GCSなど）**がバッチ処理基盤の主役として明確に位置づけられたことです。これは第2章で紹介した「ストレージがローカルディスクからリモートのオブジェクトストレージへ」という業界全体のシフトを反映しています。
...
> **ベストプラクティス**
> - バッチジョブは「入力データを変更しない（イミュータブル）」設計にすることで、失敗時の再実行や過去データの再処理（Reprocessing）を安全に行える
> - ETL（Extract–Transform–Load）パイプラインの出力は、可能な限り既存の本番データストアを直接上書きせず、新しいテーブル／パーティションとして生成してから切り替える
...
Apache Kafkaに代表される**ログ型メッセージブローカー（Log-Based Message Broker）**は、メッセージを消費後に削除する伝統的なメッセージキューとは異なり、追記型のログとしてイベントを保持します。保持されるのは無期限ではなく、**設定した保持期間（retention.ms）・サイズ上限（retention.bytes）・コンパクションポリシー（キーごとに最新値のみ残す log compaction）の範囲内**です。この範囲内であれば、複数の消費者が独立して・後からでも過去のイベントを再生できます。逆に言えば、再生したい期間を保持設定が下回っていると古いイベントは失われるため、再処理の要件から保持設定を逆算しておく必要があります。
...
> - イベント時刻と処理時刻のズレ（Late Arriving Events）を前提に、ウィンドウの許容遅延（Watermark）を設計段階で決めておく
...
> - 派生データストア（検索インデックスやキャッシュ）は「いつでも正データから再構築できる」設計にしておくと、障害時の復旧やスキーマ変更が大幅に楽になる

---

## 8. Part V：倫理と社会（第2版新設）

### 8.1 第14章：正しいことをする

倫理を独立したパートとして立てたのが第2版の構成上の変更点です。テーマ自体は第1版12章の一節「Doing the Right Thing」で扱われていましたが、AIが意思決定プロセスに深く組み込まれるようになった現状を踏まえて再編・大幅に加筆され、独立章となりました。データシステムの設計者が技術的な正しさだけでなく**倫理的な責任**も負う、という問題意識が軸になっています。
...
あわせて、PostgreSQLの拡張エコシステム（`pgvector`によるベクトル検索、`pg_search`による全文検索、`pg_duckdb`による分析）が非常に強力になっており、多くのプロダクトが「まずPostgresで拡張機能を使い、本当にスケールやコア機能として必要になった段階で専用ストア（Pinecone、Turbopufferのようなベクトル検索専業サービスなど）に移行する」という段階的な戦略を取るようになっているとの分析が示されています。
...
- [ ] 派生データ（検索インデックス、キャッシュ、DWH）を正データから再構築可能な設計にしているか
