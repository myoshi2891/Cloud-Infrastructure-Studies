...
2026年2月〜3月、約9年ぶりとなる**第2版**が Kleppmann と Riccomini の共著という形で刊行されました。ユーザーが提示したURL（`.../9781491903063/`）は2017年刊行の**第1版**のページであるため、まずは両版の違いを正確に把握しておくことが重要です。
...
| データモデル | 2章 | 3章 | GraphQL・Datalog・Property Graph・DataFrameを追加 |
...
| 倫理 | ― | 14章「正しいことをする」（新設） | AI時代のバイアス・プライバシー・説明責任を新規追加 |
...
第2版では新たに、**Datalog**（再帰的なクエリを表現できる宣言的言語）、**Event Sourcing / CQRS**（書き込みと読み取りのモデルを分離する設計パターン）、**DataFrame・行列形式**（機械学習・データ分析パイプラインで多用される表現）についても解説が拡張されています。
...
第2版で新設された注目トピックが、**Sync Engines とローカルファースト・ソフトウェア**です。これはKleppmann自身の近年の研究テーマでもあり、オフライン中でもローカルで編集を続け、オンライン復帰時に自動的に同期・マージするアプリケーション設計（Figma、Linearのようなツールで採用が進む方式）を扱っています。

> **ベストプラクティス**
> - 「強い整合性」が必須な機能（決済、在庫確保）にはシングルリーダー＋同期レプリケーション寄りの構成を選ぶ
...
**線形化可能性（Linearizability）**は、分散システムがあたかも単一のコピーしか存在しないかのように振る舞う、最も強い一貫性の保証です。強力である反面、達成のためのコストが大きく、ネットワーク分断時には可用性を犠牲にせざるを得ないというトレードオフ（いわゆるCAP的な制約）が存在します。
...
**合意アルゴリズム（Consensus）**は、複数のノードが「ある1つの値」について一致することを保証する仕組みで、リーダー選出・分散ロック・トランザクションのアトミックなコミットなど、様々な問題の基盤になります。代表的な実装にPaxos、Raft、Zabがあり、実務ではこれらを内包したZooKeeperやetcdのような「**コーディネーションサービス**」を直接利用するケースが一般的です。
...
Apache Kafkaに代表される**ログ型メッセージブローカー（Log-Based Message Broker）**は、メッセージを消費後に削除する伝統的なメッセージキューとは異なり、追記型のログとしてイベントを保持し続けます。これにより、複数の消費者が独立して・後からでも過去のイベントを再生できる柔軟性が生まれます。
...
第2版で完全に新設されたパートです。AIが意思決定プロセスに深く組み込まれるようになった現在、データシステムの設計者が技術的な正しさだけでなく**倫理的な責任**も負うという問題意識のもとに書かれています。
...
Kleppmann自身は、AIエージェントがデータベースに対して直接操作を行うようになる将来を見据え、「**AIに許可する操作は、あらかじめ明確に定義されたAPI経由に限定し、その操作が満たすべき整合性のルールを明示すべきだ**」という考えを示しています。人間とAIエージェントの双方が同じデータベースを共通の基盤としながら、互いの変更をレビュー・比較・マージできるワークフローの必要性を説いており、これは第6章のマルチリーダーレプリケーションにおける「競合解決」の考え方とも通じる発想です。
