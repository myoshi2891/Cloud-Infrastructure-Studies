...
```mermaid
flowchart TD
    A["パケットの宛先アドレスを確認"] --> B{"一致する経路が<br/>ルーティングテーブルに存在するか？"}
    B -- "存在しない" --> C["パケットを破棄し<br/>ICMP到達不能を返す"]
    B -- "1件のみ存在" --> D["その経路で転送"]
    B -- "複数存在（プロトコルが異なる）" --> E["Administrative Distance<br/>が最小の経路を採用"]
    B -- "複数存在（同一プロトコル内）" --> F["メトリック値が<br/>最小の経路を採用"]
    E --> D
    F --> D
```
...
| 802.11ax | Wi-Fi 6 / 6E | 2.4 GHz / 5 GHz（6EはGHz帯追加） | 約9.6 Gbps |
