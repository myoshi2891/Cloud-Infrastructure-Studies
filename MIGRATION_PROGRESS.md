...
`Gcp-app-dev-environment-complete-guide.html`（静的HTML・2246行）を、正準の設計パターン（NavBar + page.tsx + SetUpAnAppDevEnvironmentGuide.tsx + constants.ts + page.css + 共有 MermaidDiagram）で `app/gcl/associate-cloud-engineer/set-up-an-app-dev-environment-on-google-cloud` ルートへ完全移行する。文章・表・14個のMermaid図・コードブロック・注意書きの一切の省略・要約なしで移植。

### 完了済みステップ

- [x] **Step 1 (Red)**: `test: add failing tests for GCP app dev environment complete guide` (`__tests__/gcl/associate-cloud-engineer/set-up-an-app-dev-environment-on-google-cloud/page.test.tsx` テスト更新)
...
- [x] **Step 4 (Docs Sync & Archive)**: `chore: archive Gcp-app-dev-environment-complete-guide.html and update progress` (`Gcp-app-dev-environment-complete-guide.html` を `archive/Gcl_Archive/Associate-Cloud-Engineer/html/` へ移動、`MIGRATION_PROGRESS.md` 更新)
