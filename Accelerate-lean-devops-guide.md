...
16. [Accelerateから現在へ：2018→2025年DORAレポートの進化](#16-accelerateから現在へ2018→2025年doraレポートの進化)
...
なお、2025年のDORA調査ではAI活用の広がりを受けて5つ目の指標「Rework Rate（手戻り率）」が正式に追加され、信頼性（Reliability）が準指標として扱われるようになっています（詳細は第16章）。
...
```mermaid
flowchart TB
    WV["17. 作業の可視化<br/>（カンバン等）"] --> WIP["18. WIP制限<br/>仕掛中作業の上限を設定"]
    WIP --> Mon["16. 事業判断に資する<br/>本番モニタリング"]
    Mon --> Notify["19. プロアクティブな通知<br/>閾値超過を早期検知"]
...
flowchart TB
    ...
    Method --> PLS["PLS構造方程式モデリング<br/>（Partial Least Squares SEM）<br/>複数の潜在変数間の<br/>因果的な関係を推定"]
    PLS --> Result["技術プラクティス→SDP※→<br/>組織パフォーマンス・文化・<br/>従業員満足度への因果構造を提示"]
...
## 16. Accelerateから現在へ：2018→2025年DORAレポートの進化
...
flowchart LR
    ...
    Y2018 --> Y2021["2021年<br/>信頼性(Reliability)を<br/>準指標として追加"]
    Y2021 --> Y2025["2025年<br/>State of AI-assisted<br/>Software Development に改題<br/>5指標目 Rework Rate 追加"]
    Y2025 --> Y2026["2026年<br/>AI起因の品質劣化・<br/>手戻り増大が焦点に"]

    classDef step fill:#16233f,stroke:#7c9eff,color:#e6ebff
    class Y2014,Y2018,Y2021,Y2025,Y2026 step
```
...
| 指標 | 分類 | 概要 |
|---|---|---|
| デプロイ頻度 | 従来の4指標 | 本番リリースの頻度 |
| 変更のリードタイム | 従来の4指標 | コミットから本番稼働までの時間 |
| 変更失敗率 | 従来の4指標 | 変更が障害を招いた割合 |
| （障害からの）復元時間 | 従来の4指標 | 障害からの回復時間（"Failed Deployment Recovery Time"として再定義） |
| Rework Rate（手戻り率） | 2025年追加の第5指標 | 計画外の再デプロイ・修正対応がどれだけ発生しているか |
| Reliability（信頼性） | 準指標 | 可用性・パフォーマンス等、ユーザー体感の安定性 |

2026年に入ってからの複数の業界レポートは、AIコーディングアシスタントの普及によって「個々の開発者のアウトプット量」は増加している一方、コードレビューの滞留時間の急増やレビューなしマージの増加など、品質面の信号が悪化しつつあることを指摘しています。これはAccelerateが確立した「速度と安定性は両立する」という命題そのものへの反論ではなく、AIが導入プロセスの規律（レビュー・テスト自動化・小さなバッチ）を伴わない場合に、その両立が崩れうることを示す新たな知見と位置づけられます。
...
```mermaid
flowchart TB
    S1["① まず4つの鍵指標を計測する<br/>（現在地を可視化）"] --> S2["② バージョン管理の対象を<br/>コード以外にも拡大する<br/>（IaC・設定・スキーマ）"]
...
- [ ] 自チームの4つの鍵指標（デプロイ頻度・リードタイム・変更失敗率・MTTR）を計測できる状態にした
