...
```mermaid
flowchart TD
    A["従業員の退職・デバイス紛失/盗難が発生"] --> S["退職確定後、直ちに<br/>ユーザーアカウントを一時停止"]
    S --> B{"デバイスの所有区分"}
    ...
    H --> I["Driveデータの所有権を移転"]
    I --> J["ライセンスを解除し<br/>アカウントを保留/アーカイブ/削除"]
    J --> K["デバイス一覧から確認・削除"]
    ...
    class A eventFill
    class B,E qFill
    class S,C,D,C1,D1,F,G,H,I,J actionFill
    class K resultFill
```
...
- オフボーディング手順は「退職確定後のアカウント一時停止」「デバイスワイプ/アカウントワイプ」「サインインCookieのリセット」「Driveデータの所有権移転」「ライセンス解除・アカウントの保留orアーカイブor削除」という順序をテンプレート化し、都度の判断のばらつきを防ぐ。
- 退職が確定した時点で速やかにユーザーを一時停止し、その後にワイプ、セッション遮断、所有権移転などのデータ保全作業を行う（アクセス遮断とデータ保全の順序を誤ると、退職者による意図的なデータ持ち出しリスクが残る）。
...
flowchart TD
    ...
    C --> H{"更新チェックの<br/>間隔を調整したいか?"}
    H -->|"はい"| I["Auto-update check period<br/>を分単位で設定"]
...
- **Auto-update check period**: Chromeブラウザが更新を確認する間隔を分単位で設定する。
