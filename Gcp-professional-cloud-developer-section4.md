...
sequenceDiagram
    ...
    alt プール内に空きコネクションあり
        ...
        Conn->>DB: TLS 1.3で暗号化された<br/>認可済み接続を確立
        DB-->>Conn: 接続確立
...
Cloud StorageはgRPC・JSON API・XML APIのいずれでもアクセス可能で、C++・C#・Go・Java・Node.js・PHP・Python・Rubyのクライアントライブラリが用意されています。アプリケーションからの接続管理という観点では、Cloud SQLやFirestoreのような「コネクションの確立と維持」という概念よりも、クライアントオブジェクト（例: Pythonの`storage.Client()`）を使い回し、リクエストごとに新規クライアントを生成しないことが重要です。クライアントの内部でHTTP/2接続プーリングが行われるため、アプリケーション側で明示的なプール管理は不要ですが、クライアントインスタンスの再利用はパフォーマンス上の基本です。
...
Firestoreはコレクション/ドキュメントのモデルで、単一ドキュメントの読み書きはミリ秒単位で完了しますが、複数ドキュメントにまたがる整合性が必要な操作にはトランザクションやバッチ書き込みを使います。
...
flowchart TD
    ...
    G --> H["8. 必要に応じて配信の詳細設定を行う<br/>（Exactly-once配信・順序保証・フロー制御）"]
...
sequenceDiagram
    ...
    alt 未確認メッセージ数が上限未満
        ...
        Topic-->>Client: messageId を返却（ack）
        Client-->>Pub: 発行成功を通知
...
- **消費処理は冪等に設計する**: Pub/Subのat-least-once配信により同一メッセージが重複配信され得るため、メッセージIDによる重複排除など、何度処理されても結果が変わらない実装にします。
...
flowchart TD
    ...
    C1 -->|"Yes"| Use1["指定されたJSONキーファイル<br/>（サービスアカウントキー）を使用"]
...
| 探索順序 | 認証情報のソース | 主な用途 |
|---|---|---|
| 1 | `GOOGLE_APPLICATION_CREDENTIALS`環境変数が指すJSONキーファイル | CI/CDや他クラウド上での実行など、明示的なキー配布が必要な場合 |
...
正しく相関が設定されていれば、Cloud Trace側でスパンの詳細を表示すると関連するログエントリへのリンクが表示され、逆にLogs Explorer側では「Correlated by trace」の設定を有効にすることで、同一トレースIDを持つログエントリを親子関係のようにグループ化して表示できます。
...
従来のトラブルシューティングは、担当エンジニアが自らダッシュボード・ログ・トレースを横断的に確認し、仮説を立てて検証するという手作業が中心でした。**Gemini Cloud Assist**、特にその**Investigations**（調査）機能は、この一連のプロセスをAIが支援・自動化する仕組みです。試験範囲としての「AI支援オブザーバビリティ」は、この機能がどのようにデータを扱い、どのような制約のもとで動作するかを理解しているかを問うものです。
...
flowchart TD
    ...
    B -->|"自動"| D["バックグラウンド監視エージェントが<br/>アラートを検知して自動的に<br/>ディープダイブ調査を開始"]
...
Gemini Cloud Assist Investigationsは、GKEのアラート付きワークロード、失敗したバッチジョブ、失敗したAirflowタスクなど、対応する多くのプロダクトページから直接起動できるほか、コンソール右上のInvestigationsアイコンやモバイルアプリからも起動可能です。調査の過程で使われるOAuth 2.0トークンのアクセス範囲は、それを実行したユーザーまたはサービスアカウントが本来持つ権限の範囲に限定されており、**データを変更する目的では一切使用されません**。つまりGemini Cloud Assistは「読み取って分析する」ことに専念し、実際の是正措置（ロールバックや設定変更など）は人間の明示的な承認を経て実行される設計になっています。
