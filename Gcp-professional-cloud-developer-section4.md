...
この相関が成立するには、次の3つの条件をすべて満たす必要があります。第一に、親ログと子ログの`trace`フィールドの値が一致していること（`TRACE_ID`形式と`projects/PROJECT_ID/traces/TRACE_ID`形式のどちらでも構いませんが、両者で同じ値になっている必要があります）。第二に、親ログと子ログの`logName`が**異なる**こと（同一ログ名のエントリ同士は親子として相関されません）。第三に、親ログのタイムスタンプが子ログのタイムスタンプ**以前**であること（親のほうが後の時刻になっていると相関されません）。
...
flowchart TD
    ...
    B -->|"自動<br/>（バックグラウンド）"| D["バックグラウンド監視エージェントが<br/>アラートを検知して自動的に<br/>ディープダイブ調査を開始"]
