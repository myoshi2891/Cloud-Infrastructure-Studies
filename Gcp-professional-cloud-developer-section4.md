...
| Private IP + 直接TLS接続 | VPC内から直接データベースドライバで接続 | ネットワーク層で完結させたい特殊要件がある場合 |
...
- **Cloud SQL Language Connectorsを優先する**: 対応言語（Java/Python/Go/Node.js）では、証明書のライフサイクル管理が不要になるLanguage Connectorsを、Auth Proxyよりも先に検討します。
...
flowchart TD
    ...
    G --> H["8. 必要に応じて配信の詳細設定を行う<br/>（Exactly-once配信 ※Pullのみ・<br/>順序保証・フロー制御）"]
...
sequenceDiagram
    ...
    alt 未確認メッセージ数が上限未満
        ...
        Note over Client: 既存メッセージのackを待つ
    end
...
|---|---|---|
| Pull（プル型） | サブスクライバーが能動的にメッセージを取得しにいく。クライアントライブラリが必要 | 常時稼働するワーカー、GKE上のサービスなど |
...
**Exactly-once配信の適用範囲**：Exactly-once配信を有効にできるのは**Pullサブスクリプション（Pull API / StreamingPull）のみ**です。PushサブスクリプションおよびBigQueryサブスクリプションのようなエクスポート型では利用できないため、重複排除を配信側の保証に依存する設計はPull型に限られます。

#### ベストプラクティス

- **パブリッシャー/サブスクライバークライアントを使い回す**: リクエストごとに生成せず、アプリケーションのライフサイクル全体で単一のクライアントインスタンスを共有します。
- **消費処理は冪等に設計する**: Pub/Subのat-least-once配信により同一メッセージが重複配信され得るため、何度処理されても結果が変わらない実装にします。同一メッセージの再配信は`messageId`で弾けますが、パブリッシャー側が再試行した場合は内容が同じでも`messageId`が変わるため、注文IDのような**業務上の冪等性キー**を使った永続的な重複排除（FirestoreやCloud SQLの一意制約に処理済み記録を残す）を併用します。このとき、重複排除の記録と実際の副作用（DB更新など）は**同一トランザクションでコミット**し、「副作用だけ適用されて記録が残らない」中途半端な状態が生じないようにします。
...
- **依存関係を意識して無効化する**: あるサービスが他の有効なサービスに依存されている場合、無効化はエラーになります。依存サービスも含めて無効化したい場合は`disableDependentServices`パラメータを使います。
...
flowchart TD
    ...
    Q1 -->|"No"| Q2{"低レイテンシ・双方向<br/>ストリーミングが必要か？"}
    Q2 -->|"Yes"| GRPC["gRPCで直接呼び出す<br/>（protoベースのRPC）"]
    Q2 -->|"No"| Q3{"手元でAPIの挙動を<br/>試したいだけか？"}
...
多数の小さなリクエストを個別に送信すると、リクエストごとのオーバーヘッド（TCP接続・TLSハンドシェイク・認証検証）が積み重なりスループットが低下します。バッチ処理では複数のAPI呼び出しを1つのHTTPリクエストにまとめて送信し、往復回数を削減します。多くのGoogle Cloud APIは、リスト系メソッドに対する`batchCreate`・`batchGet`のような一括操作や、HTTPレベルのバッチエンドポイントを提供しています。
...
flowchart TD
    ...
    Retryable -->|"Yes"| MaxCheck{"最大試行回数/<br/>最大経過時間に達したか？"}
...
- **リトライは冪等な操作にのみ適用する**: GETやリストのような読み取り操作は安全にリトライできますが、POSTのような非冪等な操作をリトライする場合はリクエストIDなどで重複実行を防ぎます。
...
Section 1.2で扱った認証の基礎（Application Default Credentials、WIFなど）を、実際の「API呼び出し」というコンテキストで再確認する項目です。本番環境でCloud APIを呼び出すアプリケーションは、原則としてユーザーアカウントではなく**サービスアカウント**の権限で動作させます。ここで鍵となる概念が**Application Default Credentials**（ADC）です。
...
技術的には、Cloud LoggingのLogEntryオブジェクトが持つ`trace`フィールドと`spanId`フィールドがこの相関の鍵となります。`trace`フィールドには`TRACE_ID`（推奨フォーマット）または`projects/PROJECT_ID/traces/TRACE_ID`（レガシー形式）を設定し、`spanId`フィールドには16文字の16進数エンコードされたスパンIDを設定します。OpenTelemetryでアプリケーションを計装し、アクティブなスパンのコンテキスト内でログを出力している場合、多くのGoogle Cloudのロギングクライアントライブラリはこれらのフィールドを**自動的に**設定します。HTTPリクエストが介在する場合は、W3Cの`traceparent`ヘッダーや`X-Cloud-Trace-Context`ヘッダーの値からトレースフィールドを設定することも可能です。

正しく相関が設定されていれば、Cloud Trace側でスパンの詳細を表示すると関連するログエントリへのリンクが表示され、逆にLogs Explorer側では、「クエリ結果（Query results）」ペインの「Correlate by」メニューからトレースを選ぶことで、同一トレースIDを持つログエントリをグループ化して表示できます。この相関が成立する条件は、対象のログエントリの`trace`フィールドが`projects/PROJECT_ID/traces/TRACE_ID`形式で、かつトレースIDが一致していることです。
...
その具体例が**Gemini Cloud Assist**の**Investigations**（調査）機能です。ただしInvestigationsは執筆時点で**private preview**として提供されており、一般提供（GA）された安定機能ではありません。提供状況・UI・利用条件は変わりうるため、以下は「AI支援オブザーバビリティの考え方を示す実装例」として読み、実際の利用可否は必ず公式ドキュメントとリリースノートで確認してください。
...
flowchart TD
    ...
    B -->|"自動<br/>（private preview）"| D["バックグラウンド監視エージェントが<br/>アラートを検知して自動的に<br/>ディープダイブ調査を開始"]
    C --> E["2. Gemini Cloud Assistが<br/>OAuth 2.0トークン<br/>（実行ユーザー/SAの権限範囲内）で<br/>関連データへアクセス"]
    D --> E
    E --> F["3. ログ・メトリクス・トレース・<br/>設定変更履歴・トラブルシューティング<br/>runbookを横断的に相関分析"]
...
Investigationsが利用可能な環境では、GKEのアラート付きワークロード、失敗したバッチジョブ、失敗したAirflowタスクなど、対応するプロダクトページから直接起動できるほか、コンソール右上のInvestigationsアイコンやモバイルアプリからも起動できるとされています。アラートを起点にバックグラウンド監視エージェントが自動で調査を開始する動作も同様にprivate previewの範囲であり、いずれも安定提供機能として設計に織り込むべきものではありません。調査の過程で使われるOAuth 2.0トークンのアクセス範囲は、それを実行したユーザーまたはサービスアカウントが本来持つ権限の範囲に限定されており、**データを変更する目的では一切使用されません**。つまりGemini Cloud Assistは「読み取って分析する」ことに専念し、実際の是正措置（ロールバックや設定変更など）は人間の明示的な承認を経て実行される設計になっています。
...
- **調査を実行する権限を適切にスコープする**: 調査はユーザー/サービスアカウントの既存の権限範囲内でのみデータにアクセスするため、過度に広い権限を持つアカウントで調査を行わないよう、通常の最小権限の原則をそのまま適用します。
...
| 4.1.3 | メッセージングでの発行/消費 | Pub/Subのフロー制御、ackDeadline、at-least-once配信 | 消費側は「重複配信され得る」前提で冪等に設計する |
