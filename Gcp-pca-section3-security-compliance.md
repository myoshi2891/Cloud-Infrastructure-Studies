...
| レプリケーションポリシー | 自動（Google Cloudが管理する複数リージョンへ自動的に複製。単一リージョンが選ばれるわけではない）またはユーザー管理（保存先リージョンを明示的に指定でき、データ主権要件に対応）を選択可能[^42] |
| アクセス制御 | IAMのきめ細かいロール（Secret Accessor / Secret Version Manager等）とIAM Conditionsで、閲覧・管理・監査の職責を分離可能[^38] |
| ローテーション通知 | ローテーション予定時刻に`SECRET_ROTATE`イベントをPub/Subへ発行できる。**Secret Manager自身がローテーションを実行するわけではなく**、通知を購読したワークフロー側で新しいシークレットバージョンの作成まで行う必要がある[^42] |
...
| 組織ポリシー制約の例 | 目的 |
| --- | --- |
| `constraints/iam.disableServiceAccountKeyCreation` | ユーザー管理のサービスアカウントキーの**新規作成**を組織全体で禁止する。代替となる認証方式（サービスアカウントのリソースへの直接アタッチ、サービスアカウントのなりすまし、Workload Identity Federation）への移行を促す |
...
| 鍵とデータのロケーションを一致させる | 要件はリソース種別ごとに異なる。**リージョンリソース・ゾーンリソース**は同一リージョンまたは`global`の鍵、**マルチリージョンリソース**は対応するマルチリージョンまたはデュアルリージョンの鍵、**グローバルリソース**は`global`の鍵を使う。これによりレイテンシと主権要件を両立させる | [^30] |
...
| サービスアカウントのなりすまし（Impersonation） | 権限を持つ既存プリンシパルが、IAM Credentials APIを介して短期間有効なトークンを発行 | 委任元プリンシパルまで遡って追跡でき、キーのエクスポートより追跡性が高い |

IAM Credentials APIを利用した一時的権限昇格は、鍵のエクスポートを伴わずに済みます。ただし監査ログの読み方には注意が必要です。**なりすまし後のAPI操作では、`principalEmail`は通常「なりすまされたサービスアカウント」を示し**、「誰がなりすましたか」は`serviceAccountDelegationInfo`フィールドに委任元プリンシパルと委任チェーンとして記録されます[^14]。

さらに、**この委任情報が記録されるかどうかはサービスによって異なる**ため、`serviceAccountDelegationInfo`だけに依存した追跡は確実ではありません。Workload Identity Federation経由の外部IDまで遡る場合は、STS APIおよびIAM Credentials APIのData Accessログと、外部IdP側のログを相関させて追跡する必要があります[^14]。
...
| Build L1 | **ビルドプラットフォーム側の要件**：ソース・ビルド手順に関するプロベナンス（来歴情報）が存在し、生成されること。**プロデューサー（producer）側の要件**：生成されたプロベナンスを利用者へ配布すること。なお「ビルドプロセスの自動化」はL1の直接の要件ではない |
...
Googleは自社サービスのコンプライアンス認証取得やセキュリティ機能の提供を担いますが、それらの機能を**どう構成し、どう運用するか**は顧客の責任です。これは「共有運命（Shared Fate）」という考え方にも発展しており、Googleは単なる責任分界点の提示にとどまらず、ベストプラクティス・ブループリント・自動化ツールを通じて顧客のセキュリティ達成を積極的に支援します[^153]。
...
| アクセス制御 | IAPとIAMで、CDEへのアクセスを認証・認可し、最小権限を強制[^154] |
| 監査・監視 | Cloud Audit LogsでCDEへのアクセス操作を監査し、Security Command Centerで脆弱性・脅威の検出結果を監視[^154] |
...
本ガイドの内容は、一次情報源（Google Cloud公式ドキュメント・公式ブログ）を主軸としつつ、実装例や解説を補う二次情報源（技術ブログ等）も一部含めた以下の資料をもとに、技術検証を行った上で作成しています。二次情報源は公式ドキュメントで裏付けが取れた範囲で参照しています。
