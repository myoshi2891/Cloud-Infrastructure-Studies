...
- **バケットのロケーションと BigQuery データセットのロケーションを揃える**: 今回のラボでは 3 つの Cloud Storage バケットが `us-central1`（リージョン）、BigQuery データセットが `US`（マルチリージョン）で、両者は一致していません。`US` マルチリージョンは `us-central1` を包含するため読み書き自体は成立しますが、本番設計ではリージョンをまたぐデータ移動がレイテンシとコストの両面で不利になるため、ロケーションは設計段階で揃えておきます。
...
- **タイムアウト（HTTP 504）を「無害なエラー」として読み飛ばさないでください。** Cloud Run が処理途中で打ち切られていると、Document AI の解析が完了せず、Eventarc も配信失敗として扱います。504 を見つけたら、次を順に確認してから先へ進みます。
    1. **Eventarc の配信ステータス**: 対象トリガーの配信が成功しているか（失敗ならリトライ・デッドレターの状況）を確認します。
    2. **アーカイブ出力**: `${PROJECT_ID}-archive-invoices` バケットに処理済みファイルが出力されているか確認します。
    3. **BigQuery の行数**: `invoice_parser_results` のテーブルに想定件数の行が入っているか `bq query` 等で確認します。
- 上記のいずれかが欠けている場合は再処理が必要です。BigQuery にデータが書き込まれない場合は、あわせて環境変数（特に `PARSER_LOCATION` の大文字小文字）も再確認してください。
