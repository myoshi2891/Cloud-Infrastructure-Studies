...
- [ ] コード・インフラ定義・**機密情報を含まない**設定を単一のバージョン管理システムで一元管理しているか（認証情報・秘密鍵・APIトークン等のシークレットはバージョン管理に含めず、シークレット管理サービスで扱う）
...
| DORAメトリクス | デプロイ頻度・変更のリードタイム・変更失敗率・Failed Deployment Recovery Time（デプロイ失敗からの復旧時間。2023年に旧称「サービス復旧時間／MTTR」から、対象をデプロイ起因の失敗に限定する形で再定義）の4指標に、2024年追加の Rework Rate（手戻り率）を加えた現行5指標からなるソフトウェア提供パフォーマンスの指標群 |
