...
開発からビジネスへ向かう作業の流れ（バリューストリーム）全体のパフォーマンスを最適化する考え方です。特定のチーム（例: 開発だけ）の効率を上げても、ボトルネックが別の箇所（例: 変更管理プロセス）にあれば全体のスループットは改善しません。
...
**ベストプラクティス**：
- コード・設定・インフラ定義（IaC）を含め、すべてを単一のバージョン管理システムで管理する
...
- 未完成の機能は長期ブランチで隠すのではなく、フィーチャーフラグでコード上ONF/OFFを切り替える
...
| 手法 | 概要 | 主な利点 |
|---|---|---|
| フィーチャーフラグ | コードはデプロイ済みだが、機能のON/OFFを実行時に切り替える | デプロイと機能公開を分離でき、ロールバックが一瞬で済む |
...
DORAチームは2025年、報告書の名称を「Accelerate State of DevOps」から**「State of AI-assisted Software Development」**へと変更しました。これはDevOpsという枠を超え、AIを含む新しい働き方全般を対象とする調査へと範囲を広げたことを意味する、単なる名称変更ではない転換だと分析されています（[出典: RedMonk, "DORA 2025: Measuring Software Delivery After AI"](https://redmonk.com/rstephens/2025/12/18/dora2025/)）。あわせて、これまでの4つの主要指標（デプロイ頻度・変更のリードタイム・変更失敗率・平均修復時間）は5つ目の指標が追加され拡張されています（[出典: DORA, "2025: Year in Review"](https://dora.dev/insights/dora-2025-year-in-review/)）。
...
```mermaid
flowchart LR
    subgraph elite["エリートパフォーマー（DORA調査の分類）"]
        direction TB
        E1["デプロイ頻度: 1日に複数回"]
        E2["変更のリードタイム: 1時間未満"]
        E3["変更失敗率: 低水準"]
    ...
    subgraph low["ローパフォーマー（DORA調査の分類）"]
        direction TB
        LW1["デプロイ頻度: 半年〜1年に1回"]
        LW2["変更のリードタイム: 1〜6ヶ月"]
        LW3["変更失敗率: 高水準"]
...
2026年現在、DevOpsの実践は「プラットフォームエンジニアリング」という形で進化を続けています。Gartnerは、大規模なソフトウェアエンジニアリング組織の80%が2026年までに、アプリケーション提供のための再利用可能なサービス・コンポーネント・ツールを社内向けに提供する専任プラットフォームチームを設置すると予測しています（2022年時点の45%から増加）（[出典: Roadie.io, "Platform Engineering in 2026"](https://roadie.io/blog/platform-engineering-in-2026-why-diy-is-dead/)）。CNCFのPlatform Engineering調査によれば、2026年にはプラットフォームチームの73%がAIアシスタントを開発者ワークフローに組み込んでいるとも報告されています（[出典: LeanOps, "Platform Engineering Trends 2026"](https://leanopstech.com/blog/platform-engineering-trends-2026/)）。
...
| DORAメトリクス | デプロイ頻度・変更のリードタイム・変更失敗率・平均修復時間からなるソフトウェア提供パフォーマンスの指標群 |
