...
> **注意**: 一部の新しいモデル世代（Gemini 3系など）では、`temperature`／`top_p`／`top_k` の指定が非推奨（モデル側が自動でサンプリングを管理）とされている場合があります[^7]。ラボで指定された `model_name` のドキュメントを確認し、パラメータが有効なモデルであれば `temperature` を高め（例: 1.0〜1.5 程度）に設定するのが「最も創造的で意外性のある描写」を狙う定石です。
...
セルを再実行し、生成されたタグラインの文中に `nature` という単語（または明確にその概念を表す語）が含まれているかを確認します。含まれていなければ、Input 文言をより明示的にする、あるいは Example 側のトーンとの整合を取るなど、プロンプトを再調整します。最後にノートブックを保存します。
