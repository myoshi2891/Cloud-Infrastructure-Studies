...
| --- | --- | --- |
| Cloud Storage | オブジェクトストレージ | バックアップ、メディアファイルの配信、データレイクの構築。アクセス頻度に応じて Standard / Nearline / Coldline / Archive の4クラスが存在し、ライフサイクル管理で自動ダウングレードがベストプラクティス [2](https://cloud.google.com/learn/certification/cloud-digital-leader?hl=en)[12](https://docs.cloud.google.com/storage/docs/lifecycle) |
| Cloud SQL | リレーショナル (MySQL, PostgreSQL, SQL Server) | 強い整合性が求められるWebアプリや地域的なOLTPシステムに最適。ストレージ容量が数十TB以下の標準的な選択肢 [11](https://cloud.google.com/products/databases)[13](https://cloud.google.com/blog/topics/developers-practitioners/databases-google-cloud-part-2-options-glance/) |
| Cloud Spanner | 分散型リレーショナル | マルチリージョン規模での書き込みスケーラビリティと強いトランザクション整合性が必要な金融決済や大規模インベントリ管理 [14](https://cloud.google.com/blog/topics/developers-practitioners/your-google-cloud-database-options-explained)[15](https://cloud.google.com/blog/topics/developers-practitioners/your-google-cloud-database-options-explained) |
| Cloud Bigtable | 非リレーショナル (NoSQL ワイドカラム) | ミリ秒単位の超低レイテンシが求められるIoTセンサーデータ、時系列データ、アドテクノロジーのログなど大規模分析・運用ワークロード [14](https://cloud.google.com/blog/topics/developers-practitioners/your-google-cloud-database-options-explained)[11](https://cloud.google.com/products/databases) |
...
| BigQuery | データウェアハウス (分析向け) | ペタバイトスケールのデータ高速分析、サーバーレスEDW。SQLで対話的クエリと BigQuery ML を利用できる [11](https://cloud.google.com/products/databases)[17](https://cloud.google.com/bigquery) |
...
| Google Kubernetes Engine (GKE) | コンテナオーケストレーション | 複数のコンテナ化マイクロサービスを管理し、ステートフルなワークロードを自律的にスケール・修復。トラフィックが持続的に高い大規模エンタープライズに最適 [24](https://cloud.google.com/docs/choosing-a-compute-option)[27](https://cloud.google.com/blog/topics/developers-practitioners/where-should-i-run-my-stuff-choosing-google-cloud-compute-option) |
| Cloud Run | サーバーレスコンテナ | HTTPリクエストをトリガーにステートレスなコンテナを即時スピンアップ。スケール・トゥ・ゼロでトラフィックの変動が激しいWeb APIに最適 [25](https://cloud.google.com/run/docs/overview/what-is-cloud-run)[27](https://cloud.google.com/blog/topics/developers-practitioners/where-should-i-run-my-stuff-choosing-google-cloud-compute-option) |
| App Engine | Platform as a Service (PaaS) | コードをデプロイするだけでトラフィック分割・バージョン管理が自動化。標準的なWebアプリやモバイルアプリのバックエンドに適している [26](https://cloud.google.com/appengine/docs/the-appengine-environments)[27](https://cloud.google.com/blog/topics/developers-practitioners/where-should-i-run-my-stuff-choosing-google-cloud-compute-option) |
| Cloud Functions | Function as a Service (FaaS) | Cloud Storage や Pub/Sub などの特定イベントに反応して単一関数を実行。システム間の軽量な連携（グルーコード）や非同期バックグラウンド処理に有効 [26](https://cloud.google.com/appengine/docs/the-appengine-environments)[27](https://cloud.google.com/blog/topics/developers-practitioners/where-should-i-run-my-stuff-choosing-google-cloud-compute-option) |
...
| サポート階層 | 月額コストの体系 | SLA (レスポンスタイム) | 主要な特徴と想定ユースケース |
| --- | --- | --- | --- |
| Standard Support | $29 またはクラウド支出の 3% のいずれか高い方 [49](https://cloud.google.com/support)[50](https://cloud.google.com/support/docs/standard) | P2 (High Impact) の課題に対し 4 時間以内 | 試験的なプロジェクトや開発・テスト環境など、ミッションクリティカルでないワークロードに最適 |
| Enhanced Support | $500 またはクラウド支出の階層的割合 (10%〜3%) [49](https://cloud.google.com/support)[51](https://cloud.google.com/support/docs/enhanced) | P1 (Critical Impact) の課題に対し 1 時間以内 | 高速な対応が求められる本番環境向け。サードパーティテクノロジーのサポートや Active Assist API の利用が含まれる |
| Premium Support | $15,000 またはクラウド支出の階層的割合 (10%〜3%) [49](https://cloud.google.com/support)[52](https://cloud.google.com/support/docs/premium) | P1 (Critical Impact) の課題に対し 15 分以内 | 停止が許されないエンタープライズ規模のミッションクリティカルなワークロード向け。専任TAMが配置され、アーキテクチャガイダンスや運用健全性レビューを提供 |
