...
flowchart TB
    ...
    I --> J["2024〜2026年<br/>TotTは「Tech on the Toilet」として<br/>改称・継続、EngProd組織が運用中"]
```
...
Googleのテスト文化を象徴する概念の一つが、テストを「単体・結合・システム」のような従来の分類ではなく、**実行に必要なリソース（プロセス数・メモリ・実行時間・依存先）を基準にした「Small／Medium／Large」という3段階**で分類する方式です。
...
```mermaid
flowchart TB
    L["Large テスト（目安 約10%）<br/>複数マシン・本番同等環境で実行<br/>外部ネットワークや実サービス呼び出しを許可<br/>実行時間：数分〜"]
    M["Medium テスト（目安 約20%）<br/>単一マシン内で複数プロセスが連携<br/>localhost通信は許可、外部ネットワークは不可<br/>実行時間：数秒〜1分未満"]
    S["Small テスト（目安 約70%）<br/>単一プロセス内で完結<br/>ネットワーク・ディスクI/O・他プロセス呼び出し禁止<br/>実行時間：数十ミリ秒〜1秒未満"]
    L --> M --> S
```

| サイズ | 実行範囲 | 許可される依存関係 | 目安の実行時間 | 目安の構成比（70/20/10） |
|---|---|---|---|---|
| Small | 単一プロセス内 | 不可（ネットワーク・ディスクI/O・他プロセス禁止） | 数十ミリ秒〜1秒未満 | 約70% |
| Medium | 単一マシン内の複数プロセス | localhost通信のみ許可 | 数秒〜1分未満 | 約20% |
| Large | 複数マシン・本番同等環境 | 外部ネットワーク・実サービス呼び出し可 | 数分以上 | 約10% |
...
| 出典 | URL |
|---|---|
| Google Books（原著紹介ページ／本記事の起点） | https://books.google.co.jp/books/about/How_Google_Tests_Software.html?id=vHlTOVTKHeUC&redir_esc=y |
| Amazon（書籍情報・著者略歴） | https://www.amazon.com/Google-Tests-Software-James-Whittaker/dp/0321803027 |
| O'Reilly（書籍情報・目次インデックス） | https://www.oreilly.com/library/view/how-google-tests/9780132851572/index.html |
| InfoQ「Interview and Book Review: How Google Tests Software」（Craig Smith） | https://www.infoq.com/articles/how-google-tests-software/ |
| InfoQ 提供のサンプル第3章PDF | https://res.infoq.com/articles/how-google-tests-software/en/resources/SampleChapter-Chapter3.pdf |
| Software Testing Magazine によるレビュー | https://www.softwaretestingmagazine.com/knowledge/how-google-tests-software-by-james-whittaker/ |

### Google公式テストブログ

| 出典 | URL |
|---|---|
| Google Testing Blog トップページ（2026年7月時点の最新記事を確認） | https://testing.googleblog.com/ |
| 「Test Sizes」（Small/Medium/Largeの解説） | https://testing.googleblog.com/2010/12/test-sizes.html |
| 「What Test Engineers do at Google」 | https://testing.googleblog.com/2016/09/what-test-engineers-do-at-google.html |
| 「From QA to Engineering Productivity」（SET→SETIへの改称の経緯） | https://testing.googleblog.com/2016/03/from-qa-to-engineering-productivity.html |
| 「Evolution of GTAC and Engineering Productivity」 | https://testing.googleblog.com/2017/07/evolution-of-gtac-and-engineering.html |
| 「TotT: Avoiding Flakey Tests」（2008年） | https://testing.googleblog.com/2008/04/tott-avoiding-flakey-tests.html |
| Google Test Automation Conference（GTAC）公式概要 | https://developers.google.com/google-test-automation-conference |

### 著名な国際的エンジニア・専門メディアによる解説

| 出典 | URL |
|---|---|
| Mike Bland（元Testing Grouplet中心メンバー）「Small, Medium, Large」 | https://mike-bland.com/2011/11/01/small-medium-large.html |
| Mike Bland「Test Certified」 | https://mike-bland.com/2011/10/18/test-certified.html |
| Mike Bland「The Practice of Testing」 | https://mike-bland.com/2012/09/11/practice-of-testing.html |
| StickyMinds「Google's New Methodology for Risk-driven Testing」（ACC分析の解説） | https://www.stickyminds.com/presentation/googles-new-methodology-risk-driven-testing |
| InformIT「Using Other Influences for Planning」（ACC手法の詳細解説） | https://www.informit.com/articles/article.aspx?p=2253544&seqNum=3 |
| Software Engineering at Google（無料公開・後継書籍、第12章 Unit Testing） | https://abseil.io/resources/swe-book/html/ch12.html |
| Wikipedia「Flaky test」 | https://en.wikipedia.org/wiki/Flaky_test |
