...
- QUICはカーネルの設定項目ではなくユーザー空間の実装が輻輳制御を持つため、`sysctl`では確認できない。利用しているQUIC実装（quiche・nginx・Envoy等）が公開するメトリクスや接続ごとの診断情報（qlogのcongestion controller関連イベントなど）で、実際に使用中のアルゴリズムを確認する
