...
- Cisco: `app/cisco/` 配下（`ccna/beginner-guide/page.tsx` 完全ガイド、`ccna/automation-software-development-design/page.tsx`、`ccna/automation-application-deployment-security/page.tsx`、`ccna/automation-cisco-platforms-and-development/page.tsx`、`ccna/automation-infrastructure-and-automation/page.tsx`、`ccna/ip-connectivity-guide/page.tsx`、`ccna/ip-services-guide/page.tsx` 含む）
...
- **ユーザー手動確認ゼロ原則（手動・目視確認依頼の全廃）**:
  ...
  - テキスト全量一致の検証は `node scripts/verify-html-migration.mjs` および `bun test` による全自動 DOM 照合で 100% 合格を判定してください。
