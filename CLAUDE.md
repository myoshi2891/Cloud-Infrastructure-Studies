...
- **PCNE セクションページの `metadata.title` 規約**: `PCNE S<n>: <セクション名> | Google Cloud 認定試験対策` に統一する。`app/gcl/professional-cloud-network-engineer/layout.tsx` が `title` をプレーン文字列で定義しているため、ルート `app/layout.tsx` の `template: '%s | Cloud Infrastructure Studies'` は **PCNE サブツリーには適用されない**（テンプレート鎖が中間 layout で切れる）。したがってセクションページの title は自動接尾辞に頼れず自己完結が必須。この規約は `__tests__/gcl/professional-cloud-network-engineer/section-title-convention.test.ts` が検証する。
