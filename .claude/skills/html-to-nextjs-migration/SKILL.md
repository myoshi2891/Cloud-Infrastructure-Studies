...
- 各 `<div class="mermaid" id="diag-N">` を `<MermaidDiagram chart={DIAGRAMS['diag-…']} ariaLabel="…" preserveNaturalScale />` に置換（`.mermaid-wrap` で囲む）。
...
| Monochrome code blocks | ハイライト無しの単色 `<pre><code>` | CLI コマンドやコード各行の要素（`.code-comment`, `.code-prompt`, `.code-keyword`, `.code-command`, `.code-number`, `.code-param`）を `<span>` で装飾しカラーハイライトを全適用 |
...
| 誤ったアーカイブ先 | リポジトリ直下や `Gcl_Archive/` 単体 | `archive/Cisco/html/` と `archive/Cisco/md/` 等の階層化フォルダへ該当種別ごとに移動 |
...
- **Always apply CLI / code syntax highlighting** — コードブロックは必ず `.code-header`, `.code-line`, `.code-comment`, `.code-prompt`, `.code-keyword`, `.code-number`, `.code-param` 等のクラスでカラー装飾すること
- **Always archive original files to correct subdirectories** — Cisco資料は `archive/Cisco/html/` および `archive/Cisco/md/` へそれぞれ移動すること（削除厳禁）
...
- **Never leave code blocks unhighlighted** — 各コードブロックには構文ハイライト（色分け）を必ず適用すること
