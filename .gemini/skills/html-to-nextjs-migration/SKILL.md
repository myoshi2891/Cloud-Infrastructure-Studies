...
3. **`.gemini/rules/tdd-commit-workflow.md`** — TDD必須サイクル & コミット分割ルール
...
移行作業中は、常に `.gemini/rules/tdd-commit-workflow.md` に定められた TDD サイクル（Red → Green → Refactor → Docs）を最優先で適用しなければなりません。
...
進捗同期の手順は `.gemini/rules/migration-progress-sync.md` に従ってください。毎ページ、移行作業の完了直後に実施します。
