...
エージェントがユーザーに代わってデータへアクセスする際、**誰の権限で**アクセスしているのかを正しく伝播させることが重要です。Agent GatewayとAgent Identityを組み合わせることで、エンドユーザーの認可コンテキストをエージェント呼び出しチェーンの最後まで伝播させ、途中のエージェントが権限昇格（本来の呼び出し元より広い権限を持ってしまうこと）を起こさないようにします。
