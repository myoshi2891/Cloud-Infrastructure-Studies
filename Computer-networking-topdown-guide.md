...
| APNIC計測によるIPv6対応ネットワーク比率 | 約42〜43% | 2026年4月 |
| HTTP/3のグローバルトラフィック比率(Cloudflare計測) | 約35% | 2025年10月〜2026年 |
| RPKI(経路正当性検証)でカバーされる経路の割合 | 67.43% | 2026年6月29日 |
...
パケットがルータを通過するとき、リンク層とネットワーク層のヘッダだけが調べられ、トランスポート層より上のペイロードには触れません。この「関心の分離」こそが、レイヤードアーキテクチャの本質的な利点です。
...
```mermaid
flowchart LR
    H1["HTTP/1.1<br/>(1997年)<br/>1リクエスト=1接続<br/>または限定的なKeep-Alive"] --> H2["HTTP/2<br/>(2015年)<br/>1つのTCP接続上で<br/>複数ストリームを多重化"]
...
**2026年時点の実測データ(Cloudflare計測)**: HTTP/3はグローバルトラフィックの約35%を占めるまでに成長しており、特にパケットロス率の高いモバイル網では20〜60%のページ読み込み高速化が報告されています。
...
**DNSの安全性・プライバシーに関する2026年時点の動向**:従来のDNSは平文のUDP(ポート53)でやり取りされ、経路上の第三者に盗聴・改ざんされるリスクがありました。これに対応するため、以下の暗号化DNSプロトコルの普及が進んでいます。

| プロトコル | トランスポート | 特徴 |
|---|---|---|
| DoT (DNS over TLS) | TLS/853番ポート | OSレベルで全アプリのDNSを一括暗号化しやすい |
| DoH (DNS over HTTPS) | HTTPS/443番ポート | 通常のWeb通信に紛れるためブロックされにくい。Firefoxでは米国ユーザーの85%超が有効化(Mozilla報告) |
...
| アンプ攻撃対策 | 検証前のクライアントには受信量の3倍までしか送信しない制限を規定(RFC 9000) |
...
flowchart LR
    ...
    VALIDATOR -->|Valid / Invalid / Unknownを応答| ROUTER
    ROUTER -->|Invalidな経路を拒否| ACCEPT["採用する経路テーブル"]

    classDef rpkiFill fill:#132a4a,stroke:#7c9eff,color:#e8eefc
    class HOLDER,REPO,VALIDATOR,ROUTER,ACCEPT rpkiFill
```

**2026年時点の普及状況**: Hurricane Electricの計測によると、2026年6月29日時点でRPKIによってカバーされる経路の割合はグローバルで67.43%(158万470経路中105万6,730経路)に達し、過去最高を記録しました。Sparkle(AS6762)のような大手Tier-1トランジット事業者も2026年2月からRPKI無効経路を拒否するようになっています。
...
VLANは、部門ごとのセキュリティ境界の設定やブロードキャストトラフィックの抑制に有効であり、物理的な配線変更なしにネットワークの論理構成を柔軟に変更できるという運用上のメリットをもたらします。
...
flowchart LR
    ...
    WIFI6E --> WIFI7["Wi-Fi 7<br/>802.11be (2024正式公開)"]
...
flowchart TB
    ...
    subgraph ASYM["公開鍵暗号"]
        ...
        A1["公開鍵で暗号化、秘密鍵で復号"]
        A2["鍵配送問題を解決するが低速"]
        A3["例: RSA、楕円曲線暗号(ECDHE)"]
    end
    HYBRID["実際のTLSでは<br/>公開鍵暗号で鍵交換を行い、<br/>その後は対称鍵暗号で<br/>高速に通信する「ハイブリッド方式」を採用"]
...
**2026年時点の普及状況**: ChromeはX25519MLKEM768をバージョン124(2024年4月)からデスクトップでデフォルト有効化しており、Cloudflareの2026年テレメトリでは、Cloudflareに到達するTLS 1.3ハンドシェイクの30%以上が既に耐量子鍵交換で保護されています。Windows 11 24H2のCNG(Cryptography Next Generation)APIにもML-KEMサポートが追加されるなど、OSレベルでの対応も進んでいます。一方、耐量子**署名**(証明書の真正性検証に使う部分)は鍵サイズが大きく処理コストが高いため、鍵交換ほどには普及が進んでおらず、2026年時点でも公開の耐量子証明書はほとんど流通していない、という「非対称な移行状況」がCloudflareの分析で指摘されています。
...
| 領域 | 指標 | 数値 | 出典 |
|---|---|---|---|
| アプリケーション層 | HTTP/3グローバルトラフィック比率 | 約35% | Cloudflare Radar |
| アプリケーション層 | Firefox米国ユーザーのDoH有効化率 | 85%超 | Mozilla報告 |
| ネットワーク層 | Google経由IPv6アクセス率(世界平均) | 50.10%(2026/3/28) | Google / ISOC Pulse |
| ネットワーク層 | RPKIカバー経路割合 | 67.43%(2026/6/29) | Hurricane Electric |
| リンク・無線層 | Wi-Fi 7対応AP年間出荷予測 | 1億1,790万台 | ABI Research/WBA |
| リンク・無線層 | 6G仕様完成目標 | 2029年前半(Release 21) | 3GPP |
| セキュリティ層 | 耐量子鍵交換のTLS 1.3ハンドシェイク比率 | 30%超 | Cloudflare Radar |
...
3. **モデルベースの制御へのシフト**: BGPのRPKIやBBRの帯域推定モデルのように、静的なルールベースの制御から、実測データに基づく動的なモデルベース制御へと重心が移っています。
...
| 最長経路 / RTT | パケットが往復するのにかかる時間(Round-Trip Time) |
