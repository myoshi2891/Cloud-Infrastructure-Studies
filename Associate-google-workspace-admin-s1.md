...
| タスク | 内容 | 本ガイドの章 |
| --- | --- | --- |
| 1.1 | ユーザーライフサイクルの管理(移行・作成・プロビジョニング・SSO・同期・属性変更・削除/保留/復元/アーカイブ・所有権移転・ライセンス・パスワード) | [1.1](#11-ユーザーライフサイクルの管理) |
| 1.2 | 組織部門(OU)の設計と作成 | [1.2](#12-組織部門ouの設計と作成) |
| 1.3 | グループの管理(構造設計・配布リスト・Collaborative Inbox・ダイナミックグループ・セキュリティグループ) | [1.3](#13-グループの管理) |
| 1.4 | ドメインの管理(プライマリ/セカンダリドメインの追加と検証・ドメインエイリアス) | [1.4](#14-ドメインの管理) |
| 1.5 | 建物とリソースの管理(建物/部屋の一括作成・リソース管理・予約権限・機能設定) | [1.5](#15-建物とリソースの管理) |

---

## 目次

- [1.1 ユーザーライフサイクルの管理](#11-ユーザーライフサイクルの管理)
  - [1.1.1 移行戦略とツールの選定](#111-移行戦略とツールの選定)
  - [1.1.2 手動でのユーザーアカウント作成](#112-手動でのユーザーアカウント作成)
  - [1.1.3 プロビジョニング・デプロビジョニングの自動化](#113-プロビジョニングデプロビジョニングの自動化)
  - [1.1.4 サードパーティIDプロバイダによるプロビジョニングと認可](#114-サードパーティidプロバイダによるプロビジョニングと認可)
  - [1.1.5 基本的なSAML SSOの設定](#115-基本的なsaml-ssoの設定)
  - [1.1.6 ファーストパーティ同期ツールのユースケース](#116-ファーストパーティ同期ツールのユースケース)
  - [1.1.7 ユーザー属性の変更](#117-ユーザー属性の変更)
  - [1.1.8 削除・保留・復元・アーカイブ](#118-削除保留復元アーカイブ)
  - [1.1.9 Driveデータの所有権移転](#119-driveデータの所有権移転)
  - [1.1.10 ライセンス管理](#1110-ライセンス管理)
  - [1.1.11 パスワード管理](#1111-パスワード管理)
- [1.2 組織部門(OU)の設計と作成](#12-組織部門ouの設計と作成)
- [1.3 グループの管理](#13-グループの管理)
- [1.4 ドメインの管理](#14-ドメインの管理)
- [1.5 建物とリソースの管理](#15-建物とリソースの管理)
- [まとめ:実装チェックリスト](#まとめ実装チェックリスト)
- [参考文献](#参考文献)
...
> **出典**: [Google Workspace migration product matrix](https://support.google.com/a/answer/9413033?hl=en)
...
> **出典**: [About automated user provisioning](https://knowledge.workspace.google.com/admin/users/advanced/about-automated-user-provisioning) / [View auto-provisioning errors](https://knowledge.workspace.google.com/admin/users/advanced/view-auto-provisioning-errors)
...
2. **認可(認証)**:ユーザーがGoogleサービスにアクセスする際、SAML SSOを通じて外部IdPで認証が行われ、Googleはこれを信頼する(Google = Service Provider)。
...
2. 「Third-party SSO profiles」で **Add SAML profile** をクリックし、プロファイル名を入力。
3. IdP側から取得した **IdP entity ID**、**Sign-in page URL(SSO URL)**、**Sign-out page URL** を入力。
4. **Change password URL** にIdP側のパスワード変更URLを設定。
5. IdPから発行された署名検証用証明書をアップロード(ローテーション用に最大2枚まで登録可能)。
6. 保存後に生成される **Entity ID** と **ACS URL** をコピーし、IdP側の設定に反映する。

> **出典**: [Setting up SSO](https://knowledge.workspace.google.com/admin/apps/setting-up-sso) / [About SSO](https://support.google.com/a/answer/60224?hl=en) / [Technical overview of SAML-based SSO](https://knowledge.workspace.google.com/admin/apps/technical-overview-of-saml-based-sso)
...
> **出典**: [Set up your own custom SAML app](https://support.google.com/a/answer/6087519?hl=en)
...
| 比較項目 | GCDS | Directory Sync |
| --- | --- | --- |
| インストール | オンプレミスソフトウェアのインストールが必要 | クラウドベースのソリューションでインストール不要 |
...
| 接続方式 | 通常はLDAPサーバーと同一ネットワーク上に配置される | ADの場合はCloud VPNまたはCloud Interconnectを使用してオンプレミスのLDAPサーバーにアクセスし、Azure ADの場合は管理者のMicrosoft資格情報を使って接続する |
| 同期対象データ | 管理者を含むユーザー、グループ、カレンダーリソース、外部連絡先、パスワード | 非管理者ユーザーとグループのみ |
| 複数ソース同期 | 不可 | ADは複数ディレクトリからの同期に対応するが、Azure ADは1つのディレクトリのみ対応 |
| セットアップの複雑さ | 組織のニーズによっては非常に複雑になり得る | Google Admin consoleを使ったシンプルなセットアップ |
| 同期頻度 | 管理者が構成可能。自動化には別途スケジューリングソフトウェアが必要 | フル同期は前回の同期完了から1時間後に開始し、この間隔は変更不可 |
| トラブルシューティング | 複数サーバーからログファイルを集約する必要がある場合がある | Google Admin consoleで一元的にレポートされ、フィルタ・検索・カスタムアラートの設定が可能 |
| 属性マッピング | 最大35のシステム属性とカスタム属性をマッピング可能 | 姓・名・メールアドレス・復旧用電話番号・復旧用メールアドレスをマッピング可能 |
| OUマッピング | 指定したOUへ自動的にユーザーを配置 | ユーザーを指定したOUへマッピング可能 |
...
> **出典**: [About Google Cloud Directory Sync](https://knowledge.workspace.google.com/admin/users/about-google-cloud-directory-sync) / [Compare Directory Sync with GCDS](https://knowledge.workspace.google.com/admin/users/compare-directory-sync-with-gcds)
...
> **出典**: [Overview: Changing a Directory user's name or email address](https://knowledge.workspace.google.com/admin/users/overview-changing-a-directory-users-name-or-email-address) / [Add or delete an alternate email address (email alias)](https://knowledge.workspace.google.com/admin/users/add-or-delete-an-alternate-email-address-email-alias) / [Create custom attributes for user profiles](https://knowledge.workspace.google.com/admin/users/create-custom-attributes-for-user-profiles)
...
> **出典**: [Delete or remove a user from your organization](https://knowledge.workspace.google.com/admin/users/delete-or-remove-a-user-from-your-organization) / [Suspend a user temporarily](https://knowledge.workspace.google.com/admin/users/suspend-a-user-temporarily) / [Restore a recently deleted user](https://knowledge.workspace.google.com/admin/users/restore-a-recently-deleted-user) / [Archive former employee accounts](https://knowledge.workspace.google.com/admin/users/archive-former-employee-accounts) / [Maintain data security after an employee leaves](https://knowledge.workspace.google.com/admin/users/maintain-data-security-after-an-employee-leaves)
...
> **出典**: [Transfer Drive files to a new owner as an admin](https://knowledge.workspace.google.com/admin/drive/transfer-drive-files-to-a-new-owner-as-an-admin)
...
> **出典**: [Manage and assign licenses (GCDS)](https://knowledge.workspace.google.com/admin/users/manage-and-assign-licenses)
...
> **出典**: [Enforce and monitor password requirements for users](https://knowledge.workspace.google.com/admin/users/enforce-and-monitor-password-requirements-for-users) / [Reset a user's password](https://knowledge.workspace.google.com/admin/users/reset-a-users-password) / [Allow super administrators to recover their password](https://knowledge.workspace.google.com/admin/users/allow-super-administrators-to-recover-their-password)
...
> **出典**: [Organizational policies FAQ](https://support.google.com/a/answer/4390551?hl=en-na) / [How the organizational structure works](https://knowledge.workspace.google.com/admin/users/advanced/how-the-organizational-structure-works)

### 1.2.3 OUの作成と管理

1. `ディレクトリ > 組織部門` から対象の親OUにカーソルを合わせ、**新しい組織部門を作成** を選択。
2. OU名と説明を入力し、親OUを指定して作成する。
3. ユーザーやデバイスは作成時、または後から **ユーザーを組織部門に移動** の操作でOUへ割り当てる。
4. OUの移動・名称変更・削除を行う場合、ローカル設定(そのOUで個別に設定した項目)は保持されるが、継承設定(親から引き継いでいた項目)は新しい親OUの値に変わる点に注意する。

> **出典**: [Add an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/add-an-organizational-unit) / [Move users to an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/move-users-to-an-organizational-unit) / [Move, rename, or delete an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/move-rename-or-delete-an-organizational-unit)
...
> **出典**: [Create a group in your organization](https://knowledge.workspace.google.com/admin/groups/create-a-group-in-your-organization)
...
> **出典**: [Make a group a Collaborative Inbox](https://support.google.com/a/users/answer/10375787) / [Add features and manage conversations in Google Groups](https://knowledge.workspace.google.com/admin/groups/add-features-and-manage-conversations-in-google-groups)
...
2. **条件リスト**(例:ユーザーの部署)と **値**(具体的な部署名)を選び、条件式(クエリ)を組み立てる。クエリの最大文字数は10,000文字。
3. 複数条件は **And(&&)** または **Or(||)** で結合可能。特定条件を除外する場合は Exclude を使う。
4. ゲストユーザーは外部コラボレーターであるためダイナミックグループには既定で含まれず、クエリには自動的に `is_guest_user == false` の除外条件が付加される。
5. プレビューでメンバー候補を確認してから作成。1組織あたり最大500個のダイナミックグループを作成できる(上限緩和は個別申請が必要)。
...
> **出典**: [Manage membership automatically with dynamic groups](https://support.google.com/a/answer/10286834) / [Valid user fields for dynamic group queries](https://cloud.google.com/identity/docs/how-to/dynamic-groups-attributes)
...
> **出典**: [Control access to sensitive data with security groups](https://knowledge.workspace.google.com/admin/groups/control-access-to-sensitive-data-with-security-groups)

### 1.3.6 グループ種別の比較まとめ

| グループ種別 | メンバー管理方式 | 主な用途 | ポリシー適用への適性 |
| --- | --- | --- | --- |
| 配布リスト | 手動 | メール一括配信 | 低 |
| Collaborative Inbox | 手動 | 共有受信箱の運用(support@等) | 低〜中 |
| ダイナミックグループ | クエリによる自動 | 属性ベースの自動メンバー管理 | 高(セキュリティラベル併用時) |
| セキュリティグループ | 手動 or ダイナミック(Securityラベル併用) | 機密データ・リソースへのアクセス制御 | 最も高い(ポリシー適用の推奨形態) |
| アクセス/構成グループ | 手動 or ダイナミック | サービスのオン/オフ・設定のカスタマイズ | 中〜高 |
...
2. Googleが提示するTXTレコードの値を、ドメインレジストラのDNS設定に追加する。
3. DNSの伝播(通常は数分〜最大8時間程度)を待ち、Admin console側で **確認** をクリックして検証を完了する。
4. メール送受信を行う場合は、別途MXレコードの設定も必要になる(ドメイン所有権の検証とメールルーティングの設定は別の作業)。

> **出典**: [Add a user alias domain or secondary domain](https://support.google.com/a/answer/7502379?hl=en) / [FAQ for multiple domains](https://support.google.com/a/answer/175747?hl=en)
...
> **出典**: [Change your primary domain for Google Workspace](https://support.google.com/a/answer/7009324?hl=en) / [Limitations with multiple domains](https://support.google.com/a/answer/182081?hl=en)
...
> **出典**: [FAQ for multiple domains](https://support.google.com/a/answer/175747?hl=en)
...
> **出典**: [Create buildings, features & Calendar resources](https://support.google.com/a/answer/1033925?hl=en)
...
| 項目 | 内容 |
| --- | --- |
| リソースタイプ | 会議室(Conference room)、その他(社用車、備品等) |
| 建物・フロア | 所属する建物とフロア(構造化リソースを使う場合、Calendarの自動室提案機能に活用される) |
| 収容人数 | 会議室の場合、参加人数に応じた検索・自動提案に使われる |
| 説明・機能 | ホワイトボード、モニター、ビデオ会議設備など |
...
> **出典**: [Create buildings, features & Calendar resources](https://support.google.com/a/answer/1033925?hl=en) / [Set up Google Calendar room booking suggestions](https://support.google.com/a/answer/9025584)
...
> **出典**: [Approve or deny Calendar room & resource bookings](https://knowledge.workspace.google.com/admin/calendar/approve-or-deny-calendar-room-and-resource-bookings) / [Share room and resource calendars](https://knowledge.workspace.google.com/admin/calendar/share-room-and-resource-calendars)
...
2. **管理 > リソースの機能を管理** から **機能を追加** をクリック。
3. 機能名(例:「ホワイトボードあり」「車椅子対応」「収容人数20名以上向け設備」)を入力して保存。
4. 作成した機能は、各リソースの編集画面から個別に紐づける。ユーザーはリソース検索時にこれらの機能で絞り込める。
...
- [Google Workspace migration product matrix](https://support.google.com/a/answer/9413033?hl=en)
- [About automated user provisioning](https://knowledge.workspace.google.com/admin/users/advanced/about-automated-user-provisioning)
- [View auto-provisioning errors](https://knowledge.workspace.google.com/admin/users/advanced/view-auto-provisioning-errors)
- [Setting up SSO](https://knowledge.workspace.google.com/admin/apps/setting-up-sso)
- [About SSO](https://support.google.com/a/answer/60224?hl=en)
- [Technical overview of SAML-based SSO](https://knowledge.workspace.google.com/admin/apps/technical-overview-of-saml-based-sso)
- [Set up your own custom SAML app](https://support.google.com/a/answer/6087519?hl=en)
- [About Google Cloud Directory Sync](https://knowledge.workspace.google.com/admin/users/about-google-cloud-directory-sync)
- [Compare Directory Sync with GCDS](https://knowledge.workspace.google.com/admin/users/compare-directory-sync-with-gcds)
- [Overview: Changing a Directory user's name or email address](https://knowledge.workspace.google.com/admin/users/overview-changing-a-directory-users-name-or-email-address)
...
- [Create custom attributes for user profiles](https://knowledge.workspace.google.com/admin/users/create-custom-attributes-for-user-profiles)
- [Delete or remove a user from your organization](https://knowledge.workspace.google.com/admin/users/delete-or-remove-a-user-from-your-organization)
- [Suspend a user temporarily](https://knowledge.workspace.google.com/admin/users/suspend-a-user-temporarily)
- [Restore a recently deleted user](https://knowledge.workspace.google.com/admin/users/restore-a-recently-deleted-user)
- [Archive former employee accounts](https://knowledge.workspace.google.com/admin/users/archive-former-employee-accounts)
- [Maintain data security after an employee leaves](https://knowledge.workspace.google.com/admin/users/maintain-data-security-after-an-employee-leaves)
- [Transfer Drive files to a new owner as an admin](https://knowledge.workspace.google.com/admin/drive/transfer-drive-files-to-a-new-owner-as-an-admin)
- [Manage and assign licenses (GCDS)](https://knowledge.workspace.google.com/admin/users/manage-and-assign-licenses)
...
- [Reset a user's password](https://knowledge.workspace.google.com/admin/users/reset-a-users-password)
- [Allow super administrators to recover their password](https://knowledge.workspace.google.com/admin/users/allow-super-administrators-to-recover-their-password)

### 組織部門(OU)

- [Organizational policies FAQ](https://support.google.com/a/answer/4390551?hl=en-na)
- [How the organizational structure works](https://knowledge.workspace.google.com/admin/users/advanced/how-the-organizational-structure-works)
- [Add an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/add-an-organizational-unit)
- [Move users to an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/move-users-to-an-organizational-unit)
- [Move, rename, or delete an organizational unit](https://knowledge.workspace.google.com/admin/users/advanced/move-rename-or-delete-an-organizational-unit)
...
- [Add features and manage conversations in Google Groups](https://knowledge.workspace.google.com/admin/groups/add-features-and-manage-conversations-in-google-groups)
- [Manage membership automatically with dynamic groups](https://support.google.com/a/answer/10286834)
- [Valid user fields for dynamic group queries](https://cloud.google.com/identity/docs/how-to/dynamic-groups-attributes)
- [Control access to sensitive data with security groups](https://knowledge.workspace.google.com/admin/groups/control-access-to-sensitive-data-with-security-groups)

### ドメイン管理

- [Add a user alias domain or secondary domain](https://support.google.com/a/answer/7502379?hl=en)
- [FAQ for multiple domains](https://support.google.com/a/answer/175747?hl=en)
- [Limitations with multiple domains](https://support.google.com/a/answer/182081?hl=en)
- [Change your primary domain for Google Workspace](https://support.google.com/a/answer/7009324?hl=en)

### 建物とリソース

- [Create buildings, features & Calendar resources](https://support.google.com/a/answer/1033925?hl=en)
- [Set up Google Calendar room booking suggestions](https://support.google.com/a/answer/9025584)
- [Approve or deny Calendar room & resource bookings](https://knowledge.workspace.google.com/admin/calendar/approve-or-deny-calendar-room-and-resource-bookings)
- [Share room and resource calendars](https://knowledge.workspace.google.com/admin/calendar/share-room-and-resource-calendars)
