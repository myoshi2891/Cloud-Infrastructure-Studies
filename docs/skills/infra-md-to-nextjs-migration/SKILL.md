...
description: Complete workflow for migrating Markdown study-guide content to Next.js App Router page.tsx in this repository (Infra). Covers the TDD workflow (constants.ts → page.tsx → test), JSX card structure, SVG attribute conversion, TypeScript strict mode pitfalls, and test-text matching rules. Use this skill when the user asks to migrate markdown to Next.js, implement study content, add sections, add cards to page.tsx, or migrate exam content to App Router.
---
