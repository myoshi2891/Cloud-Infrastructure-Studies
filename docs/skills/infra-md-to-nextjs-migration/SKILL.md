...
- **コンポーネント分割とCSS Modules**: `page.tsx` は長大になるため必ず `components/sections/` へ分割し、スタイリングには `*.module.css` を用いてカプセル化する。複数セクションで共通の CSS モジュール（`SectionBase.module.css`等）を使用する場合は、クラス名のエクスポート問題を回避するため CSS での `@import` は使わず、TSX 内で直接 `import baseStyles` のように読み込むこと。
