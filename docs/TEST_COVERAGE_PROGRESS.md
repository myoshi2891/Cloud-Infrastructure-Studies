...
[coverage-dashboard.html](coverage-dashboard.html)（2026年8月14日更新）のデータを基に、現在のテスト網羅性と進捗状況を整理したレポートです。
...
- **ソースファイル総数:** 503 ファイル
- **テストカバー済みソース数:** 134 ファイル
- **全体カバレッジ達成率:** **27%**
- **テストファイル総数:** 132 ファイル (Vitest 4.x + Playwright 1.x)
...
> **集計スコープ:** 本マトリクスは `scripts/lib/classifier.mjs` の `DOMAINS` / `domainOf()` が分類する 9 ドメイン **413 ファイル（カバー済み 105 ファイル / 25%）** のみを対象とします。全体サマリー（503 ファイル / 134 ファイル）との差分 **90 ファイル（うちカバー済み 29 ファイル）** は、現時点で `domainOf()` が `null` を返す未分類ソースです。これらをドメイン行として集計するには `classifier.mjs` の `DOMAINS` / `domainOf()` へのドメイン追加が必要です。
...
| **Google Workspace Admin (AGWA)** | 8 / 25 | **32%** | ⚠️ 32% | ❌ 0% | ⚠️ 4% | ❌ 0% | ✅ 実装 | ✅ 実装 | ✅ 実装 | ✅ 実装 |
...
- **現状**: PCNE（52%）や PCNE Step-by-Step（55%）など、比較的新しい移行セクションでは TDD 開発ルールに従って単体テストが整備されています。AGWA は Section 1〜4 を含む9テストファイルで、移行本文、テーブル列数、リンク順序、Section 2のスクロール末尾判定を検証しています。CCNA（33%）/ DevNet（10%）は独立ドメインとして集計されています。
...
AGWA の移行検証は Section 1〜4 の正規配置と共有抽出ヘルパーへ同期済みです。最新の実行記録は 2026-08-14T23:20:20Z、対象コミット `52bfd16`、スコープはサンドボックス指定の `npm test` による全体Vitestで、128ファイル・1100件が成功しました。`MIGRATION_PROGRESS.md` の記録は標準のbunコマンドを用いた先行実行（2026-08-14T16:50:02Z、`5bf90b5`、127ファイル・1099件）であり、実行条件が異なるため別結果として扱います。

直近のレビュー対応では、ミラー比較をraw Bufferへ変更し、移行テストテンプレート、Mermaidモック、インベントリ実行契約を3系統で同期しました。対象テストは35件および94件が成功し、全体ESLintも成功しています。標準全体テストは987件が成功し、既存スモーク期待値1件と実行環境の `window.localStorage` 不備による30件が失敗しているため、次回はテスト環境と既存スモーク期待値の切り分けを優先候補に含めてください。
