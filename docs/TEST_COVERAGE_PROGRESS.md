...
- **ソースファイル総数:** 503 ファイル
- **テストカバー済みソース数:** 133 ファイル
- **全体カバレッジ達成率:** **26%**
- **テストファイル総数:** 131 ファイル (Vitest 4.x + Playwright 1.x)
...
> **集計スコープ:** 本マトリクスは `scripts/lib/classifier.mjs` の `DOMAINS` / `domainOf()` が分類する 9 ドメイン **413 ファイル（カバー済み 104 ファイル / 25%）** のみを対象とします。全体サマリー（503 ファイル / 133 ファイル）との差分 **90 ファイル（うちカバー済み 29 ファイル）** は、現時点で `domainOf()` が `null` を返す未分類ソースです。これらをドメイン行として集計するには `classifier.mjs` の `DOMAINS` / `domainOf()` へのドメイン追加が必要です。
...
| **Google Workspace Admin (AGWA)** | 7 / 25 | **28%** | ⚠️ 28% | ❌ 0% | ⚠️ 4% | ❌ 0% | ✅ 実装 | ✅ 実装 | ✅ 実装 | ✅ 実装 |
...
- **現状**: PCNE（52%）や PCNE Step-by-Step（55%）など、比較的新しい移行セクションでは TDD 開発ルールに従って単体テストが整備されています。AGWA は Section 1〜4 を含む8テストファイルで、移行本文、テーブル列数、リンク順序を検証しています。CCNA（33%）/ DevNet（10%）は独立ドメインとして集計されています。
...
AGWA の移行検証は Section 1〜4 の正規配置と共有抽出ヘルパーへ同期済みで、全体 Vitest は127ファイル・1099件が成功しています。
