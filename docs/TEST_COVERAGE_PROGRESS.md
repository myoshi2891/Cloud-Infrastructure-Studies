...
[coverage-dashboard.html](coverage-dashboard.html)（2026年8月15日更新）のデータを基に、現在のテスト網羅性と進捗状況を整理したレポートです。
...
- **ソースファイル総数:** 603 ファイル
- **テストカバー済みソース数:** 162 ファイル
- **全体カバレッジ達成率:** **27%**
- **テストファイル総数:** 155 ファイル (Vitest 4.x + Playwright 1.x)
...
> **集計スコープ:** 本マトリクスは `scripts/lib/classifier.mjs` の `DOMAINS` / `domainOf()` が分類する 9 ドメイン **423 ファイル（カバー済み 108 ファイル / 26%）** のみを対象とします。全体サマリー（603 ファイル / 162 ファイル）との差分 **180 ファイル（うちカバー済み 54 ファイル）** は、現時点で `domainOf()` が `null` を返す未分類ソースです。これらをドメイン行として集計するには `classifier.mjs` の `DOMAINS` / `domainOf()` へのドメイン追加が必要です。
