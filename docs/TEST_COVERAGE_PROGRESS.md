...
> **集計スコープ:** 本マトリクスは `scripts/lib/classifier.mjs` の `DOMAINS` / `domainOf()` が分類する 9 ドメイン **423 ファイル（カバー済み 108 ファイル / 26%）** のみを対象とします。全体サマリー（513 ファイル / 137 ファイル）との差分 **90 ファイル（うちカバー済み 29 ファイル）** は、現時点で `domainOf()` が `null` を返す未分類ソースです。これらをドメイン行として集計するには `classifier.mjs` の `DOMAINS` / `domainOf()` へのドメイン追加が必要です。
