...
- **ソースファイル総数:** 176 ファイル
- **テストカバー済みソース数:** 57 ファイル
- **全体カバレッジ達成率:** **32%**
- **テストファイル総数:** 61 ファイル (Vitest 4.x + Playwright 1.x)
...
| **共通 (components / lib / navigation)** | 8 / 11 | **73%** | ⚠️ 64% | ⚠️ 27% | ⚠️ 9% | ⚠️ 9% | ❌ 0% | ❌ 0% | ❌ 0% | ❌ 0% |
...
### ② Integration（結合テスト）
- **現状**: 共通ロジックである `lib/navigation.test.ts` と `lib/recentPages.test.ts` の 2 件のみが部分的（27%）にカバーされています。
...
4. **対象優先度**:
   「4. 優先度別ネクストアクション」に定義された **🔴 P0: 共通/libカバレッジ補強**（`lib/utils.ts`, `lib/navigation.ts`, `lib/recentPages.ts` の Vitest によるテスト追加）から順に、**🟡 P1**（CDL, PCNE, AGWAのE2E / 単体テスト追加）へと進めてください。

それでは、最も優先度の高い **🔴 P0: 共通 / lib カバレッジ補強** のテスト整備から作業を開始してください。
