...
- ASCIIアートではなく、**Mermaid記法によるフローチャート**と**Markdownの表**のみで図解しています。GitHubやVS Code、多くのMarkdownビューアでそのままレンダリングされます。
...
これらは、いずれも「ユーザーからの入力・リクエストを検証せずに信頼してしまう」ことが根本原因である点が共通しています。
...
**この章のポイント**
- XSS・SQLインジェクション・CSRFはいずれも「入力・リクエストを検証せず信頼する」ことが根本原因。
