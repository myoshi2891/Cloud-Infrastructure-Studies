...
flowchart TD
    ...
    Q6 -->|UDP/QUIC| Q6U{"プロトコル固有の応答が返るか<br/>(DNS: dig @宛先 名前 / HTTP/3: curl --http3-only -v)<br/>汎用の nc -u は無応答でも判定不能"}
    Q6T -->|不可| F5["トランスポート層(TCP)の問題の可能性<br/>宛先サービスがLISTENしているか<br/>経路のFW・ACL・SGで遮断されていないか<br/>戻り経路のルーティングを確認"]
    Q6U -->|不可| F5U["トランスポート層(UDP/QUIC)の問題の可能性<br/>UDPはコネクションを張らないため<br/>telnet/ncのTCP失敗を障害の根拠にしない<br/>該当UDPポート・QUIC(UDP/443)の許可と<br/>UDPを落とすミドルボックスの有無を確認"]
...
- UDPベースのサービスはプロトコル固有の応答で判定する。DNSなら`dig @<宛先> <名前>`、HTTP/3なら`curl --http3-only -v`でQUICハンドシェイクの成否を見る。`--http3`はHTTP/2・HTTP/1.1へ暗黙にフォールバックするため、QUIC疎通の判定には必ず`--http3-only`を使う。`curl --http3-only`が失敗して`curl --http2`が成功する場合は、経路上でUDP/443が遮断されている可能性を疑う。汎用の`nc -u`の無応答は判定不能であり、遮断の根拠にはならない
