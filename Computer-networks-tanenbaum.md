...
- [はじめに: なぜコンピュータネットワークを学ぶのか](#はじめに-なぜコンピュータネットワークを学ぶのか)
- [学習の進め方(ベストプラクティス)](#学習の進め方ベストプラクティス)
- [ステップ0: 全体像を掴む ― インターネットはどう成り立っているか](#ステップ0-全体像を掴む-インターネットはどう成り立っているか)
- [ステップ1: 物理層 ― ビットを電気信号・光・電波に変える](#ステップ1-物理層-ビットを電気信号光電波に変える)
- [ステップ2: データリンク層 ― 隣接ノード間でフレームを確実に届ける](#ステップ2-データリンク層-隣接ノード間でフレームを確実に届ける)
- [ステップ3: メディアアクセス制御(MAC)サブレイヤー ― 誰が話す番かを決める](#ステップ3-メディアアクセス制御macサブレイヤー-誰が話す番かを決める)
- [ステップ4: ネットワーク層 ― 異なるネットワークをまたいで届ける](#ステップ4-ネットワーク層-異なるネットワークをまたいで届ける)
- [ステップ5: トランスポート層 ― エンドツーエンドの信頼性](#ステップ5-トランスポート層-エンドツーエンドの信頼性)
- [ステップ6: アプリケーション層 ― 人とアプリのためのプロトコル](#ステップ6-アプリケーション層-人とアプリのためのプロトコル)
- [ステップ7: ネットワークセキュリティ ― 守るべきものと手段](#ステップ7-ネットワークセキュリティ-守るべきものと手段)
- [ステップ8: 2026年の最新動向 ― 今のインターネットはどう変わったか](#ステップ8-2026年の最新動向-今のインターネットはどう変わったか)
- [ステップ9: トラブルシューティングの基本 ― 現場で使う一次切り分け](#ステップ9-トラブルシューティングの基本-現場で使う一次切り分け)
- [学習ロードマップ](#学習ロードマップ)
- [ベストプラクティス チェックリスト](#ベストプラクティス-チェックリスト)
- [用語集](#用語集)
- [参考文献](#参考文献)
...
## ステップ2: データリンク層 ― 隣接ノード間でフレームを確実に届ける

データリンク層は、同一のリンク(同一セグメント)上にある隣接ノード間で、ビット列を「フレーム」という単位にまとめ、誤り検出を行いながら届ける役割を担います。ネットワーク層のIPアドレスとは異なり、この層では**MACアドレス(Media Access Control address)**という48ビットの物理アドレスが使われます。

### フレーミングと誤り検出

送信側はビット列の区切り(フレームの開始・終了)を明示し、受信側が正しく1フレームを切り出せるようにします。また伝送中のビット誤りを検出するために、多くの実装で**CRC(巡回冗長検査、Cyclic Redundancy Check)**が使われます。CRCは送信側でフレーム内容から計算した検査値をフレーム末尾に付加し、受信側で同じ計算を行って一致するかを確認する仕組みです。
...
Wi-Fi Allianceの発表を集計した業界データでは、2026年通年でWi-Fi 7対応機器の出荷が世界で約11億台に達すると見込まれており、企業向けアクセスポイントの出荷台数も2024年の2,630万台から2026年には1億台超規模へと急拡大しています。
...
UDPの上に信頼性・輻輳制御・暗号化を独自に構築したのが、後述するQUIC(HTTP/3の輸送プロトコル)です。TCPのようにOSカーネル内蔵ではなくアプリケーション層で実装できるため、輻輳制御アルゴリズムの改善サイクルを速められる利点があります。
