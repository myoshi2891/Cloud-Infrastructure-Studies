...
- **将来の拡張余地を残す**: `/24`（251〜252台分）で十分か、将来的にオートスケーリングでVM数が増える見込みがあるかを事前に検討する
...
| `fw-allow-rdp` | 65535 | Ingress | Allow | すべてのインスタンス | `0.0.0.0/0` | TCP:3389 |
| `fw-allow-icmp` | 1000 | Ingress | Allow | すべてのインスタンス | `10.10.10.0/24`, `10.10.20.0/24` | ICMP |

> **手順書の表記についての注意**: 配布された手順書ではルール2の送信元範囲が `0.0.0.0/24` と記載されていますが、これは `0.0.0.0`〜`0.0.0.31` というごく一部のアドレスしか含まない範囲になってしまい、実質的に誰からもRDP接続できません。「あらゆる送信元」を意図しているのであれば、正しくは `0.0.0.0/0` です。本ガイドの表・コマンド例は意図通りに動作する `0.0.0.0/0` で統一しています。
...
gcloud compute firewall-rules create fw-allow-rdp \
  --network=vpc-net-challenge --direction=INGRESS --action=ALLOW \
  --rules=tcp:3389 --source-ranges=0.0.0.0/0 --priority=65535
...
上記の3ルールは、あくまでチャレンジラボの採点条件を満たすための構成です。送信元範囲に0.0.0.0/0を使用すると、Google Cloud外部を含むすべてのIPv4送信元からのトラフィックが許可されてしまいます。本番環境でこの設定をそのまま使うと、インターネット全体からSSH/RDPへの総当たり攻撃を受けられる状態になります。実務でのあるべき姿は「7. 本番環境向けベストプラクティス総まとめ」で詳しく解説します。
...
| 項目 | ラボの構成 | 本番環境での推奨構成 |
|---|---|---|
| SSH/RDPの送信元範囲 | `0.0.0.0/0`（全世界から許可） | IAP TCP-forwardingの利用を前提に送信元範囲をIAPの範囲(35.235.240.0/20)へ限定し、0.0.0.0/0からのポート22/3389へのIngressは拒否する |
...
| デフォルトVPC | 未使用（今回はカスタムVPCを新規作成） | 既定の「default」ネットワークには過度に許可的なルールが事前定義されているため、削除してカスタムVPCから設計を始める |
| 可観測性 | ログ未設定 | VPC Flow LogsとFirewall Rules Loggingを有効化し、脅威検知や障害調査に必要な可視性を確保する。すべてのDenyイベントをログに残しつつ、重要度の低いAllowイベントはサンプリングしてコストを抑える運用が現実的 |
...
- [ ] `default`ネットワークを無効化・削除し、カスタムVPCへ統一したか
