...
flowchart TD
    ...
    B --> C{"ルーティングテーブルに\n一致するエントリはあるか？"}
    C -- "一致するエントリあり" --> D["該当エントリのネクストホップ／\n出力インターフェースへ転送"]
    C -- "一致するエントリなし" --> E{"ゲートウェイ・オブ・ラストリゾート\n（デフォルトルート）は設定されているか？"}
...
```mermaid
flowchart TD
    Start(["複数の経路候補がある"]) --> Step1{"① 最長プレフィックスマッチ\n（Longest Prefix Match）\nより長い（詳細な）プレフィックスは？"}
    Step1 -- "プレフィックス長が最長の\nエントリが1つに絞れる" --> UseRoute["その経路を採用"]
    Step1 -- "プレフィックス長が同じ経路が\n複数残る" --> Step2{"② アドミニストレーティブ\nディスタンス（AD）\nより小さいADは？"}
    Step2 -- "ADが最小のエントリが\n1つに絞れる" --> UseRoute
...
stateDiagram-v2
    [*] --> Down
    Down --> Init : Hello受信前
    Init --> TwoWay : 自分のRouter IDが\n相手のHelloに含まれるのを確認
    TwoWay --> ExStart : マスター/スレーブを決定
    ExStart --> Exchange : DBD（データベース記述）\nパケットを交換
    Exchange --> Loading : LSR/LSUで\n詳細情報を要求
    Loading --> Full : LSDB（リンクステート\nデータベース）が完全に同期
    Full --> [*]
```
...
graph TB
    Host["ホストPC\nデフォルトゲートウェイ：\n仮想IP 192.168.1.254"]
    VIP(("仮想IP\n192.168.1.254"))
    RA["ルータA（Active/Master）\n実IP：192.168.1.1"]
    RB["ルータB（Standby/Backup）\n実IP：192.168.1.2"]
