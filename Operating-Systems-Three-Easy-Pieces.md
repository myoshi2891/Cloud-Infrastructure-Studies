...
これら3つに加えて、原著には2020年に追加された**セキュリティ章（Web版限定）**があり、本ガイドでは第6部として扱います。
...
flowchart LR
    ...
    Journal["ジャーナリング方式<br/>(先行記録/write-ahead logging)<br/>更新内容を先に<br/>ジャーナル領域へ記録し<br/>クラッシュ時はジャーナルのみ確認"]
...
sequenceDiagram
    ...
    Note over Journal: この時点で電源断が起きても<br/>ジャーナルから復旧可能
...
NFSの設計で特に重要なのが**サーバーステートレス性（server statelessness）**という考え方です。サーバーはクライアントごとの状態を保持しないため、サーバークラッシュ後の復旧が単純になりますが、キャッシュ一貫性の保証は弱くなります（`close-to-open`セマンティクスなどで妥協）。

| 概念 | 内容 |
|---|---|
| ステートレスプロトコル | 各要求が単独で完結し、サーバーはクライアントの状態を記憶しない |
| べき等性（idempotency） | 同じ要求を複数回送っても結果が変わらない設計にすることで、再送による不整合を防ぐ |
...
パスワード認証の実装では、平文保存を避けるために**ソルト付きハッシュ（salted hash）**を使う、というのが実務上の要点です。ソルトはレインボーテーブル攻撃（事前計算済みハッシュ表による総当たり攻撃）を無効化します。
...
|---|---|
| 第7〜9章（CPUスケジューリング全般） | Linuxの標準スケジューラはCFS（Completely Fair Scheduler）から、**Linux 6.6でマージされたEEVDF（Earliest Eligible Virtual Deadline First、Peter Zijlstra氏らが開発）**へ置き換わっている。さらに**Linux 6.12でマージされたsched_ext**（eBPFベースのプラガブルスケジューラ基盤）により、MLFQやくじ引きスケジューリングに近い独自ポリシーをカーネル再コンパイルなしに実験できる時代になった |
...
- [ ] 第1部：バッチ処理→マルチプログラミング→タイムシェアリングの歴史的必然性を理解した
- [ ] 第2部：`fork()`/`exec()`/`wait()`を使った簡単なCプログラムを自分で書いて動かした
- [ ] 第2部：制限付き直接実行における「trap」と「タイマー割り込み」の役割の違いを説明できる
- [ ] 第2部：FIFO/SJF/STCF/RR/MLFQ/くじ引きスケジューリングの長所・短所を比較できる
- [ ] 第3部：アドレス空間・セグメンテーション・ページングの発展の流れを図で説明できる
- [ ] 第3部：TLBミス時に何が起きるかをステップごとに説明できる
- [ ] 第3部：FIFO/LRU/クロックアルゴリズムのページ置換方針を比較できる
- [ ] 第4部：レースコンディションが起きるコード例を自分で書き、ロックで修正できる
- [ ] 第4部：条件変数を`while`文で使うべき理由を説明できる
- [ ] 第4部：デッドロックの4条件をすべて挙げられる
- [ ] 第5部：HDDのシーク・回転待ちとRAIDレベルごとのトレードオフを説明できる
- [ ] 第5部：ジャーナリングによってクラッシュ一貫性がどう保証されるかを説明できる
- [ ] 第5部：NFSとAFSの設計思想の違い（ステートレス vs キャッシュ重視）を説明できる
- [ ] 第6部：認証・アクセス制御・暗号の役割の違いを説明できる
- [ ] 第7部：xv6-riscvのソースコードを一部読み、OSTEPの概念と対応づけられた
- [ ] 第8部：EEVDF/sched_ext/io_uringなど、OSTEPの概念が現代システムでどう発展しているか説明できる
