...
- **最小権限の原則に従い、ビルド専用のサービスアカウントを指定する**：デフォルトではCompute Engineのデフォルトサービスアカウントが使われますが、セキュリティ姿勢を高めるために`--build-service-account`フラグで専用のサービスアカウントを明示的に指定することが推奨されています。
- **本番運用ではソースデプロイをCI/CDの入口として使う**：Cloud RunコンソールUIの「継続的デプロイの設定」機能や、手動で作成するCloud Buildトリガーを使えば、mainブランチへのpushをトリガーに自動でビルド・デプロイされる仕組みを構築できます。裏側の仕組みは`--source`デプロイと同じBuildpacksパイプラインです。
...
1. **トリガーに使うサービスアカウントを用意する**：Eventarcトリガーは、Cloud Runサービスを呼び出すためのアイデンティティとしてサービスアカウントに紐づけられます。デフォルトではCompute Engineのデフォルトサービスアカウントが使われますが、独自のサービスアカウントを作成し、`roles/run.invoker`ロールを付与するのがベストプラクティスです。
...
5. **リトライの既定値を確認する**：リトライの既定動作は作成方法によって異なります。`gcloud eventarc triggers create`には`--max-retry-attempts`のようなリトライ用フラグはなく、gcloud CLIおよびコンソールのEventarcページで作成したトリガーは**リトライが有効**な状態になります。一方、Cloud Runページから作成したトリガーは**1回だけ配信する**（リトライしない）のが既定です。リトライ挙動を変更したい場合は、トリガーに紐づくPub/Subサブスクリプションの再試行ポリシーを更新します。
...
flowchart TB
    ...
    APIGEE_POLICIES --> PRIVATE["プライベート接続経路<br/>（サーバーレスNEG + 内部ALB /<br/>PSCサービスアタッチメント /<br/>Apigeeエンドポイントアタッチメント）"]
    PRIVATE --> RUN["Cloud Runサービス<br/>（バックエンドAPI）"]
...
1. **Cloud Runサービスを非公開（認証必須）でデプロイする**：外部からの直接アクセスを防ぐため、Cloud Runサービス自体は未認証呼び出しを許可せず、Apigeeが使うサービスアカウントにのみ`roles/run.invoker`を付与します。
2. **Apigeeプロキシを作成し、Cloud RunのURLをターゲットエンドポイントとして設定する**：APIプロキシのターゲットにCloud RunサービスのURLを指定し、ターゲットの認証にはGoogle署名付きIDトークン（ターゲットエンドポイントの`<Authentication>`配下の`<GoogleIDToken>`）を構成します。`<Audience>`には呼び出し先Cloud RunサービスのURLをそのまま指定し、Apigeeが使うサービスアカウントには`roles/run.invoker`を付与します。なお、外部クライアントをOAuth 2.0やAPIキーで保護する話（手順4）は、このCloud Runターゲット認証とは別レイヤーの設定です。
...
6. **プライベート接続の経路を選択する**：インターネットを経由させたくない場合、VPCネットワークピアリングやCloud Interconnectだけではマネージドサービスである Cloud Run へ直接到達できません。Apigeeランタイムから Cloud Run へのプライベート経路として、①Cloud RunをバックエンドにしたサーバーレスNEG＋内部Application Load Balancer、②Private Service Connect（PSC）のサービスアタッチメント、③Apigeeのエンドポイントアタッチメント のいずれかを明示的に構成します。VPCピアリングやCloud Interconnectは、これらの経路やオンプレミス接続を成立させるための土台であり、それ単体が Cloud Run への直接経路になるわけではありません。
...
- **機微なデータはインターネットを経由させない**：可能であればCloud InterconnectなどによるVPCピアリングを利用し、Apigeeとバックエンドの通信をプライベートネットワーク内に閉じます。
...
2. **最小・最大レプリカ数を設定する**：`minReplicas`と`maxReplicas`で、スケーリングの下限・上限を明示します。上限を設定しないと、予期しない急激なトラフィック増加時にコストが際限なく膨らむリスクがあります。
