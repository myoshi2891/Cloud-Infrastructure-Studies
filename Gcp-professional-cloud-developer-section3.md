...
2. **Pub/Subトリガーを作成する**：`gcloud eventarc triggers create`コマンド（または既存トピックを利用する`gcloud run deploy`側の統合フロー）で、対象のPub/Subトピックと呼び出し先のCloud Runサービスを結びつけます。
...
5. **リトライを有効化する**：配信に失敗した場合に再送してほしい場合は「失敗時に再試行」オプションを有効にします。デフォルトの動作は「1回だけ配信し、リトライしない」です。
...
| 配信フォーマット | CloudEvents形式に標準化される | Pub/Subのメッセージ形式（Base64エンコードされたJSON） |
...
flowchart TB
    ...
    APIGEE_POLICIES --> PRIVATE["プライベート接続<br/>（VPCピアリングなど）"]
...
2. **Apigeeプロキシを作成し、Cloud RunのURLをターゲットエンドポイントとして設定する**：APIプロキシのターゲットにCloud RunサービスのURLを指定し、OAuth2で保護されたサービスアカウント経由で呼び出すよう構成します。
...
6. **プライベート接続を検討する**：インターネットを経由させたくない場合は、ApigeeのランタイムをVPCネットワークピアリングでテナントプロジェクトに接続し、Cloud Interconnectを使ってオンプレミスとの直接接続を確立することもできます。
...
- **サービスアカウントは最小権限で運用し、ルートごとにマッピングする**：Apigeeが使うサービスアカウントには`roles/run.invoker`のような最小限のロールのみを付与し、どのサービスアカウントがどのCloud Runルートを呼び出せるかを明確に管理します。
...
`stabilizationWindowSeconds`は、直近のウィンドウ期間内で計算された「望ましいレプリカ数」の中から最大値を採用することで、メトリクスの一時的な揺らぎによるレプリカ数の頻繁な増減（フラッピング）を防ぐ仕組みです。デフォルトでは、スケールダウン時に300秒（5分）のウィンドウが適用され、スケールアップ時にはウィンドウなし（即座に反映）が既定値です。

| 設定項目 | 役割 | デフォルト値の挙動 |
| --- | --- | --- |
| `scaleUp.stabilizationWindowSeconds` | スケールアップ判断を安定させるための遡及期間 | 0秒（安定化なし、即座にスケールアップ） |
| `scaleDown.stabilizationWindowSeconds` | スケールダウン判断を安定させるための遡及期間 | 300秒（過去5分間の最大推奨値を採用） |
