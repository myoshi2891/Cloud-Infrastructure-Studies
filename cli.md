...
- **前提環境**：Linux / macOS の bash または zsh（Windowsの場合は WSL2 を推奨）
...
| 一時ファイルを確認しながら削除する | `find . -name "*.tmp" -print0 \| xargs -0 -p rm` | `-p` で1件ずつ実行確認が出るため学習中も安全 |
...
```mermaid
flowchart TD
    A[Webサービスに繋がらない] --> B[ping host で疎通確認]
    B --> C{応答がある?}
    C -->|No| D[ネットワーク経路 or ファイアウォールを疑う]
    C -->|Yes| E[dig host で名前解決を確認]
    E --> F{正しいIPに解決されている?}
    F -->|No| G[DNS設定を疑う]
