...
- 「そもそもレガシーコードを生み出さないためにどう開発するか」に主眼を置きつつ
- 「すでにできあがってしまったレガシーコードを、変更が必要になったときにどう安全に改善するか」も併せて扱う

という点にあります。タイトルの「脱却」は、悪くなったコードとの戦い方だけを指すのではなく、悪くなる土壌そのものを断つための考え方を指しています。
...
9つのプラクティスは、エクストリーム・プログラミング（XP）、スクラム、リーンといった複数のアジャイル手法で培われてきた実践を、著者が初学者にも扱いやすい形に整理し直したものです。全体は大きく4つの目的グループに分けて理解すると把握しやすくなります。
...
> この図は9つのプラクティスを目的別にグループ化したものであり、番号順の依存関係や実施順序を示すものではありません。どのプラクティスから着手するかはチームの状況に応じて選べます。
...
| 8 | Implement the Design Last | 設計は最後に行う | 設計を先に固定せず、小さなサイクルの中で発展させる |
...
開発チームに実装手順（How）だけを渡すと、要求の背景が失われ、状況が変わったときに柔軟に対応できません。本書では、依頼する側が「What（何を）」「Why（なぜ）」「For Whom（誰のために）」を先に共有し、「How（どう作るか）」は開発者側の責任領域として委ねることを勧めています。具体的な手段として、プロダクトオーナーを置くこと、ユーザーストーリーで要求を表現すること、受け入れ基準を明確にして自動化することが挙げられています。
...
このプラクティスは、Robert C. Martin（Uncle Bob）の『Clean Code』や Miško Hevery の「Clean Code Talks」といった考え方へのオマージュとして、著者が独自に提唱した **CLEAN** という頭字語で整理されています。5つの性質を1語で思い出せるため、コードレビューや設計判断のチェックリストとして使いやすい形にまとめられています。
...
最後のプラクティスは、既存のコードに手を入れる場面での指針です。技術的負債を「投資」と「借金」のどちらとして扱うかという視点、コードに変更が必要になったときの判断基準、そしてオープン・クローズドの原則に沿ったリファクタリング技法などが紹介されています。Bernsteinは、テストのないレガシーコードに対しては、まず現状の振る舞いをそのまま固定する「ピニングテスト（pinning test）」を書き、それを安全網としてから構造に手を入れることを勧めています。ピニングテストは、対象の内部を理解しないまま現在の出力をそのまま期待値として固定する粗粒度のテストで、リファクタリングの前後で全体の振る舞いが変わっていないことだけを検知するための一時的な安全網です。これに対して特性化テスト（characterization test）は、Michael Feathersが『Working Effectively with Legacy Code』（レガシーコード改善ガイド）で示した、既存コードの実際の振る舞いを調べながら「仕様を発見して記述していく」テストであり、目的も粒度も異なるため両者を同義語としては扱いません。

同書の「継ぎ目（Seam）」もこれらとは別の概念で、**コードそのものを直接編集せずに振る舞いを変更できる箇所**を指します。テスト用に依存を差し替えるのはSeamの代表的な用途のひとつにすぎず、Seam＝テスト専用の仕組みではありません。手順としては、まず対象にピニングテストを追加できないかを試し、データベースや外部APIなどの依存に強く結びついていてテストを書けない場合に、その依存を分離するためのSeamを導入してからピニングテストを成立させます。

```mermaid
flowchart TB
    L1["テストのないレガシーコードに直面"] --> L2{"そのままピニングテストを<br/>追加できるか"}
    L2 -->|依存が邪魔で書けない| S1["継ぎ目 Seam を導入して<br/>依存を分離する"]
    S1 --> L3
    L2 -->|書ける| L3["ピニングテストで<br/>現状の振る舞いを固定"]
...
| 4週目 | プラクティス5・8・9 | CLEANの観点でコードレビューし、既存コードの一部にピニングテストを追加してから小さくリファクタリングする |
