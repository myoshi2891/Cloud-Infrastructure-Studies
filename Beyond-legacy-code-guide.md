...
最後のプラクティスは、既存のコードに手を入れる場面での指針です。技術的負債を「投資」と「借金」のどちらとして扱うかという視点、コードに変更が必要になったときの判断基準、そしてオープン・クローズドの原則に沿ったリファクタリング技法などが紹介されています。Bernsteinは、テストのないレガシーコードに対しては、まず現状の振る舞いをそのまま固定する「ピニングテスト（pinning test、特性化テストとも呼ばれる）」を書き、それを安全網としてから構造に手を入れることを勧めています。なお、テスト用に依存を差し替えられる箇所を指す「継ぎ目（Seam）」は本書ではなくMichael Feathersの『Working Effectively with Legacy Code』（レガシーコード改善ガイド）で提唱された概念であり、ピニングテストと組み合わせて使うと効果的です。
