...
| **コンピューティングサービス** | **抽象化レベル** | **主なユースケースとアーキテクチャの適合性** | **チームダイナミクスと運用のベストプラクティス** |
| --- | --- | --- | --- |
| **Compute Engine** | IaaS (仮想マシン) | 特殊なOS要件を持つレガシーアプリ、SAP HANA、Windowsベースのシステムなどのリホスト移行。 | インフラ、OSパッチ、セキュリティを自社で完全に管理できる運用チームが必要である 。 |
| **Google Kubernetes Engine (GKE)** | CaaS (コンテナ) | 複雑なマイクロサービスアーキテクチャ、ハイブリッド/マルチクラウド環境での一貫した運用。 | クラスター管理、高度なCI/CDパイプライン、カスタムセキュリティ設定を運用する大規模チーム向け 。 |
| **Cloud Run** | サーバーレスコンテナ | Webサイト、API、データ処理アプリ、Webhooksなど、ステートレスなコンテナ化ワークロード。 | インフラ管理を抽象化し、コードの開発とデプロイに集中したい小規模〜中規模チームに最適 。 |
| **App Engine** | PaaS | 標準的なWebアプリケーション、モバイルアプリのバックエンド。 | インフラ構築のトイルを排除し、ソースコードのデプロイのみでアプリケーションを公開したい場合に利用する 。 |
| **Cloud Functions** | FaaS | GCSのアップロード検知、データストリームの非同期処理、Webhook連携などのイベント駆動型処理。 | 単一のロジック実行に特化しており、常時稼働させる必要がない軽量なタスクの自動化に用いる 。 |
