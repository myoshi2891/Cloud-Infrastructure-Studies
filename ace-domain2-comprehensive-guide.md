# Domain 2: クラウドソリューションの計画・実装

## Google Cloud ACE 試験対策 — 完全詳細解説ガイド

> **対象読者**: Google Cloud 初学者〜中級者  
> **試験配点**: 全体の約 **30%**（Standard Exam 最大配点ドメイン）  
...
1. [コンピューティングサービス選定の全体像](#ch1)
2. [Compute Engine (GCE) の詳細](#ch2)
3. [Spot VM とプリエンプティブル VM](#ch3)
4. [Managed Instance Group (MIG)](#ch4)
5. [Google Kubernetes Engine (GKE)](#ch5)
6. [Cloud Run](#ch6)
7. [Cloud Functions](#ch7)
8. [Cloud Storage（オブジェクトストレージ）](#ch8)
9. [ブロックストレージとファイルストレージ](#ch9)
10. [データベースサービス完全選定ガイド](#ch10)
11. [VPC ネットワーク設計](#ch11)
12. [Shared VPC と VPC Peering](#ch12)
13. [Cloud NAT と Cloud DNS](#ch13)
14. [ロードバランサの選定と設定](#ch14)
15. [Infrastructure as Code (Terraform)](#ch15)
16. [Domain 2 試験対策まとめ](#ch16)

---

<div id="ch1"></div>

## Chapter 1: コンピューティングサービス選定の全体像

### 1.1 「どのコンピューティングサービスを使うか？」を判断するフローチャート
...
### 1.2 コンピューティングサービス比較表

| サービス | 管理レベル | 起動速度 | コスト | 最適なユースケース |
| ---------- | ----------- | ---------- | -------- | ------------------ |
...
## Chapter 2: Compute Engine (GCE) の詳細

### 2.1 Compute Engine とは？
...
> ⚠️ **重要**: Local SSD のデータは VM が**停止・削除されると消えます**。永続化が必要なデータには使用しないこと。

---

### 2.4 VM へのセキュアなアクセス管理
...
gcloud compute firewall-rules create allow-http-https \
  ...
  --target-tags=web-server \  # タグで対象を絞る
...
gcloud compute firewall-rules create allow-ssh-bastion \
  ...
  --rules=tcp:22 \
  --target-tags=bastion \
  --source-ranges=203.0.113.0/24  # 会社の出口IP のみ
```
...
## Chapter 3: Spot VM とプリエンプティブル VM
...
> ⚠️ **重要**: Spot VM はいつでも停止される可能性があります。ステートフルなアプリや停止が許されないサービスには使用禁止！

---

### 3.2 Spot VM と旧 Preemptible VM の違い
...
> 🔗 **参考**: https://cloud.google.com/blog/products/compute/google-cloud-spot-vm-use-cases-and-best-practices

---

<div id="ch4"></div>

## Chapter 4: Managed Instance Group (MIG)
...
gcloud compute instance-groups managed rolling-action start-update my-mig \
  ...
  --max-surge=1 \        # 同時に追加できる新VM数
  --max-unavailable=0    # 同時に停止できるVM数（0=サービス継続）
```

---

### 4.3 ゾーン MIG vs リージョン MIG
...
| # | ベストプラクティス |
| --- | ------------------- |
| 1 | 本番環境は**リージョン MIG** でゾーン障害に備える |
| 2 | **ヘルスチェック + 自動ヒーリング**を必ず設定する |
| 3 | Spot VM との組み合わせでコスト削減（`--provisioning-model=SPOT`） |
| 4 | オートスケーリングの `max-num-replicas` に上限を設ける（コスト暴走防止） |
| 5 | ローリングアップデートで **`max-unavailable=0`** を設定してゼロダウンタイム更新 |

---

<div id="ch5"></div>

## Chapter 5: Google Kubernetes Engine (GKE)

### 5.1 Kubernetes の基本概念（GKE を理解するための前提知識）
...
### 5.2 GKE の 2 つのモード: Autopilot vs Standard
...
gcloud container clusters create-auto my-autopilot-cluster \
  ...
  --enable-private-nodes \          # プライベートクラスタ（外部IP なし）
...
アプリが Google Cloud API（Secret Manager、Cloud Storage など）にアクセスする際の認証方法です。
...
metadata:
  name: my-pod
spec:
  serviceAccountName: my-ksa  # この KSA を指定
  containers:
  - name: my-container
    image: my-image
```
...
| スケーリング種別 | 対象 | 仕組み |
| ---------------- | ------ | -------- |
| **HPA**（Horizontal Pod Autoscaler） | Pod 数 | CPU/メモリ/カスタムメトリクスに基づいてPod を増減 |
| **VPA**（Vertical Pod Autoscaler） | Pod のリソース量 | CPU/メモリのリクエストを自動調整 |
| **Cluster Autoscaler** | ノード数 | Pod がスケジュールできない時にノードを追加 |
| **KEDA** | Pod 数 | Pub/Sub キューなど外部メトリクスに基づいてスケール |
...
| 6 | **リソースリクエストとリミットを必ず設定** | Pod のリソース競合・コスト最適化 |
...
> 🔗 **参考 (Autopilot vs Standard)**: https://docs.cloud.google.com/kubernetes-engine/docs/resources/autopilot-standard-feature-comparison
...
gcloud run deploy my-service \
  ...
  --region=asia-northeast1 \
  --platform=managed \
  --execution-environment=gen2 \  # 第2世代を明示指定
  --vpc-egress=all-traffic \       # すべてのトラフィックをVPCに流す
  --network=my-vpc \
  --subnet=my-subnet
```
...
gcloud run deploy my-service \
  ...
  --vpc-egress=private-ranges-only \  # プライベートIPのみVPC経由
  --network=my-vpc \
  --subnet=my-subnet
```
...
| 5 | **Secret Manager から環境変数を注入**してシークレット管理 | コードや環境変数にシークレットを直書きしない |
| 6 | **カナリアデプロイ**でリリースリスクを軽減 | 問題を早期発見・即時ロールバック |
...
### 7.1 Cloud Functions とは？

Cloud Functions はイベントに応じてコードの断片（関数）を実行するサーバーレスプラットフォームです。
...
| メモリ | 最大 8GB | 最大 32GB |
| CPU | リクエスト中のみ | 常時 |
| ベース | Cloud Run Functions（Cloud Run 上で動作） |

---

### 7.3 Cloud Functions の代表的な使用例
...
def process_image(cloud_event):
    ...
    # Artifact Registry の Cloud Vision API で画像分析
...
### 7.4 Cloud Functions vs Cloud Run の使い分け
...
#### ✅ ベストプラクティス: Cloud Functions

| # | ベストプラクティス |
| --- | ------------------- |
| 1 | **第2世代を使用**（実行時間60分・高並列性） |
| 2 | **関数は単一責任の原則**（1関数 = 1つのことだけ処理） |
| 3 | **シークレットは Secret Manager から取得**（環境変数に直書き禁止） |
| 4 | **VPC コネクタ**でプライベートリソースに接続 |
| 5 | **冪等性（Idempotency）の確保**（同じイベントを複数回受けても安全） |
...
## Chapter 8: Cloud Storage（オブジェクトストレージ）

### 8.1 Cloud Storage とは？

Cloud Storage は画像・動画・ログ・バックアップなど「非構造化データ」を保存するためのオブジェクトストレージです。
...
| クラス | GB 単価 | 取り出し料金 | 最小保存期間 | アクセス頻度の目安 |
| -------- | -------- | ------------ | ------------ | ----------------- |
...
gcloud storage buckets create gs://my-app-bucket \
  ...
  --uniform-bucket-level-access         # 推奨: 統一バケットレベルアクセス
```

---

### 8.4 オブジェクトライフサイクル管理（OLM）
...
# バージョニングが有効な場合の非最新バージョン管理
# （古いバージョンは30日後に Nearline → 90日後に削除）
```
...
> ⚠️ **バケット再作成の仕様**: バケットを削除後 10 分以内に同名バケットを作成しようとすると 404 エラーが発生します。
...
| 4 | **バージョニングを有効化**し、非最新版には OLM を設定 | 誤削除対策・コスト管理 |
...
## Chapter 9: ブロックストレージとファイルストレージ

### 9.1 Persistent Disk（永続ディスク）
...
### 9.2 Filestore（マネージドNFSファイルシステム）
...
## Chapter 10: データベースサービス完全選定ガイド

### 10.1 データベース選定のフローチャート
...
gcloud sql instances create my-postgres \
  ...
  --region=asia-northeast1 \
  --availability-type=REGIONAL \  # HA 構成
  --backup-start-time=03:00 \
  --no-assign-ip \                # 外部IP なし（プライベートIP のみ）
  --network=my-vpc
```
...
#### Cloud SQL vs Cloud Spanner の使い分け

| 判断基準 | Cloud SQL | Cloud Spanner |
| --------- | ----------- | -------------- |
| 規模 | 中規模まで | 大規模・超大規模 |
| グローバル分散 | ❌（リードレプリカは可能） | ✅ |
| 水平スケール | ❌ | ✅ |
...
#### Firestore vs Cloud Bigtable の使い分け

| 判断基準 | Firestore | Cloud Bigtable |
| --------- | ----------- | --------------- |
| データ規模 | GB〜TB | TB〜PB |
| アクセス | ドキュメント単位 | 行/列単位 |
...
> 🔗 **参考**: https://dataroots.io/blog/best-practices-for-choosing-a-database-on-google-cloud-platform-gcp

---

<div id="ch11"></div>

## Chapter 11: VPC ネットワーク設計

### 11.1 Google Cloud VPC の特徴
...
gcloud compute firewall-rules create allow-web \
  ...
  --action=ALLOW \
  --rules=tcp:80,tcp:443 \
  --target-tags=web-server \      # タグを持つVMにのみ適用
  --source-ranges=0.0.0.0/0
...
gcloud compute firewall-rules create allow-app-from-lb \
  ...
  --source-tags=load-balancer     # LB タグからのみ
```
...
## Chapter 12: Shared VPC と VPC Peering

### 12.1 Shared VPC（共有 VPC）
...
#### Shared VPC vs VPC Peering の使い分け

| 判断基準 | Shared VPC | VPC Peering |
| --------- | ----------- | ------------- |
...
## Chapter 13: Cloud NAT と Cloud DNS
...
gcloud compute routers nats create my-nat \
  --router=my-router \
  --region=asia-northeast1 \
  --nat-all-subnet-ip-ranges \          # VPC 内すべてのサブネットをNAT
  --auto-allocate-nat-external-ips      # 外部 IP を自動割り当て
```
...
| # | ベストプラクティス |
| --- | ------------------- |
| 1 | **すべての VM に外部 IP を割り当てない**（Cloud NAT でアウトバウンドを確保） |
| 2 | **ポート使用率を Cloud Monitoring で継続監視** |
| 3 | **`--min-ports-per-vm`** を接続数に応じて適切に設定 |

> 🔗 **参考**: https://cloud.google.com/blog/products/networking/6-best-practices-for-running-cloud-nat
...
gcloud dns managed-zones create on-prem-forwarding \
  ...
  --visibility=private \
  --networks=my-vpc \
  --forwarding-targets=10.0.0.53  # オンプレの DNS サーバー IP
...
> 🔗 **参考**: https://docs.cloud.google.com/dns/docs/best-practices

---

<div id="ch14"></div>

## Chapter 14: ロードバランサの選定と設定
...
### 14.2 Application Load Balancer（L7）
...
### 14.3 Network Load Balancer（L4）

#### Proxy Network LB vs Passthrough Network LB
...
### 14.4 コンプライアンス要件とロードバランサの選択

> ⚠️ **試験頻出**: データ主権・コンプライアンス要件がある場合
...
## Chapter 15: Infrastructure as Code (Terraform)

### 15.1 Terraform の基本概念
...
### 15.3 State ファイルの管理（最重要）

#### State ファイルとは？

```
Terraform は「terraform.tfstate」というファイルで
「コードと実際のGCPリソースの対応関係」を管理します
...
> ⚠️ **絶対禁止**: `terraform.tfstate` を手動で編集してはいけません！
...
terraform {
  backend "gcs" {
    bucket  = "my-terraform-state-bucket"  # State 保存用バケット
...
> 🔗 **参考**: https://cloud.google.com/blog/products/devops-sre/using-the-cloud-foundation-toolkit-with-terraform/

---

<div id="ch16"></div>

## Chapter 16: Domain 2 試験対策まとめ
...
- [ ] State ファイルは **Cloud Storage のリモートバックエンド**に保存
- [ ] State ファイルを **手動編集禁止**
- [ ] `terraform plan -out=tfplan` → `terraform apply tfplan` の順番
- [ ] CI/CD の認証は JSON キーではなく **Workload Identity/ADC**
- [ ] Terraform 1.5以降は `import` ブロックで既存リソースを取り込み

---

### 16.3 推奨学習リソース（Domain 2）

| リソース | URL |
| ---------- | ----- |
| **ACE 試験公式ページ** | https://cloud.google.com/learn/certification/cloud-engineer?hl=en |
| **Compute Engine ドキュメント** | https://docs.cloud.google.com/compute/docs |
| **OS Login 設定** | https://docs.cloud.google.com/compute/docs/oslogin/set-up-oslogin |
| **Spot VM の作成と使用** | https://docs.cloud.google.com/compute/docs/instances/create-use-spot |
| **Spot VM のベストプラクティス** | https://cloud.google.com/blog/products/compute/google-cloud-spot-vm-use-cases-and-best-practices |
...
| **データベース選定ガイド** | https://cloud.google.com/blog/topics/developers-practitioners/your-google-cloud-database-options-explained |
...
| **Cloud NAT のベストプラクティス** | https://cloud.google.com/blog/products/networking/6-best-practices-for-running-cloud-nat |
| **Cloud DNS のベストプラクティス** | https://docs.cloud.google.com/dns/docs/best-practices |
...
| **GKE Autopilot の選択** | https://oneuptime.com/blog/post/2026-02-17-how-to-choose-between-gke-autopilot-and-standard-mode-for-your-workload-requirements/view |

---

> 📝 **Domain 2 学習の最終アドバイス**
>
> Domain 2 は ACE 試験の中で**最大配点（≈30%）**です。
>
> 特に以下の点を徹底的に習得してください：
>
